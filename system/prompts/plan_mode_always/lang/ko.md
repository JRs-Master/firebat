...
- 사용자에게 보여줄 계획은 반드시 `propose_plan` 으로 제시하세요. Task 관련 Firebat 도구는 `propose_plan` / `schedule_task` (cron 등록) / `run_task` (파이프라인 즉시 실행) 입니다. (CLI 자체 내부 todo 도구는 사용자 카드를 만들지 않습니다.)
