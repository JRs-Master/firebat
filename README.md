<p align="center">
  <img src="app/icon.svg" width="80" alt="Firebat Logo" />
</p>

<h1 align="center">Firebat</h1>

<p align="center">
  <em>Just Imagine. Firebat Runs.</em>
</p>

<p align="center">
  Self-hosted AI workspace with web UI that builds, runs, and automates from a single prompt.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Next.js-16-black?logo=nextdotjs" alt="Next.js" />
  <img src="https://img.shields.io/badge/TypeScript-6-blue?logo=typescript" alt="TypeScript" />
  <img src="https://img.shields.io/badge/LLM-Multi--Provider-4285F4" alt="Multi-Provider LLM" />
  <img src="https://img.shields.io/badge/MCP-1.29-purple" alt="MCP" />
  <img src="https://img.shields.io/badge/SQLite-3-003B57?logo=sqlite" alt="SQLite" />
  <img src="https://img.shields.io/badge/License-Private-red" alt="License" />
</p>

---

## What is Firebat?

Firebat은 **대화 한 마디로 웹 앱을 만들고, 자동화하고, 운영하는** 개인 설치형 AI 플랫폼입니다.

```
"날씨 앱 만들어줘" → AI가 코드 작성 → 페이지 배포 → 크론으로 매시간 업데이트 → 카톡으로 알림
```

하나의 프롬프트가 **설계 → 구현 → 배포 → 스케줄링 → 알림**까지 관통합니다.

---

## Architecture

```
┌────────────────────────────────────────────────────────────┐
│                 app/api/  (Primary Adapter)                │
│                Next.js Route Handlers · Auth               │
├────────────────────────────────────────────────────────────┤
│                                                            │
│   ┌────────────────────────────────────────────────────┐   │
│   │                FirebatCore (Facade)                │   │
│   │                                                    │   │
│   │  AI · Storage · Page · Project · Module            │   │
│   │  Task · Schedule · Secret · MCP · Capability · Auth│   │
│   │                    11 Managers                     │   │
│   └────────────────────┬───────────────────────────────┘   │
│                        │ Ports (Interface)                 │
│   ┌────────────────────┴───────────────────────────────┐   │
│   │                 infra/  (Adapters)                 │   │
│   │                                                    │   │
│   │  Storage · Log · Sandbox · LLM · Network           │   │
│   │  Cron · Database · Vault · MCP Client · Auth       │   │
│   │                     10 Adapters                    │   │
│   └────────────────────────────────────────────────────┘   │
│                                                            │
└────────────────────────────────────────────────────────────┘
```

**Hexagonal Architecture** — Core는 순수 비즈니스 로직만 담당하고, 모든 I/O는 Infra 어댑터가 처리합니다.

| 원칙 | 설명 |
|---|---|
| **Core 순수성** | `fs`, `fetch`, DB 라이브러리 등 I/O를 Core에서 직접 호출하지 않음 |
| **Port & Adapter** | 10개 인터페이스(Port)를 통해서만 Infra와 통신 |
| **에러 캡슐화** | Infra는 절대 throw하지 않고, `InfraResult<T>`로 반환 |
| **Facade 패턴** | 모든 API route는 `getCore()` 싱글톤을 통해서만 접근 |

---

## Features

### AI Function Calling Pipeline

```
사용자 프롬프트
    ↓
[Function Calling] 각 프로바이더 네이티브 도구 호출 (멀티턴 루프)
    ↓
[Tool Execution] Core 도구 + 동적 MCP 도구 실행
    ↓
[Streaming] SSE 스트리밍 (thinking/text 실시간 전달)
    ↓
[Report] 결과 로깅 + 프론트엔드 타이핑 효과
```

- **멀티 프로바이더**: OpenAI GPT-5.4 / Anthropic Claude 4 / Google Gemini 3 / GCP Vertex AI — 동일한 `ILlmPort` 인터페이스, config JSON 하나로 모델 추가
- **CLI 모드**: 구독 기반 (Claude Pro/Max, ChatGPT Plus/Pro, Google AI Pro) — API 키 없이 로컬 CLI 자식 프로세스로 실행
- **스트리밍 응답**: `onChunk` 콜백 → SSE `chunk` 이벤트로 토큰/생각 단위 실시간 표시
- **Core 도구**: 파일 CRUD, 페이지 관리, 모듈 실행, 스케줄링, 시크릿, MCP 호출, 인라인 컴포넌트 렌더링
- **자동/수동 실행 정책**: 되돌리기 어려운 작업은 확인 요청, 나머지는 자동 실행
- **Thinking/Reasoning 통합**: 프로바이더별 플래그 자동 매핑 (`reasoning.effort` / `thinkingConfig` / `thinking.budget_tokens` / `--effort`)

### Scheduling & Automation

- **3가지 모드**: 반복(cron), 1회 예약(runAt), 딜레이(delaySec)
- **파이프라인**: MCP 조회 → LLM 요약 → 모듈 발송 같은 복합 작업을 사전 컴파일
- **4가지 파이프라인 단계**: `EXECUTE`, `MCP_CALL`, `NETWORK_REQUEST`, `LLM_TRANSFORM`
- **영속 저장**: PM2 재시작 시 자동 복원
- **동적 타임존**: 설정에서 변경 가능

### MCP (Model Context Protocol)

**MCP 서버** — 외부 AI 도구가 Firebat을 조작

| 모드 | 용도 | 인증 |
|---|---|---|
| **stdio** | Claude Code, Cursor 등 로컬 AI | SSH 키 |
| **Streamable HTTP** | VS Code, Antigravity 등 원격 연결 | Bearer 토큰 |

```json
{
  "mcpServers": {
    "firebat": {
      "url": "https://your-server.com/api/mcp",
      "headers": {
        "Authorization": "Bearer fbt_your_token_here"
      }
    }
  }
}
```

18개 도구 제공: 페이지 CRUD, 파일 CRUD, 모듈 실행, 프로젝트 관리, 크론 관리, MCP 도구 호출 등

**MCP 클라이언트** — Firebat이 외부 MCP 서버를 호출
- Gmail, Slack, 카카오톡 등 외부 서비스 연동
- AI가 자동으로 도구를 인식하고 호출

### Sandbox Execution

- **언어 중립**: Python, JavaScript, PHP, Rust, WASM, Shell
- **자동 패키지 설치**: `config.json`의 `packages` 필드에서 의존성 자동 설치
- **시크릿 주입**: Vault에서 API 키 조회 → 환경변수로 격리 전달 (AI는 키 값을 모름)
- **타임아웃**: 30초

### Built-in Components

PageSpec JSON으로 정의하면 자동 렌더링. 채팅 렌더 도구(`render_*`)로 AI가 직접 호출 가능:

`Header` · `Text` · `Card` · `Grid` · `Form` · `Image` · `Button` · `Table` · `Html` · `Divider` · `List` · `Slider` · `Tabs` · `Accordion` · `Progress` · `Badge` · `Alert` · `Callout` · `Carousel` · `Countdown` · `Chart` · `Metric` · `Timeline` · `Compare` · `KeyValue` · `StatusBadge` · `StockChart`

### Capability-Provider System

같은 기능을 수행하는 여러 모듈을 묶고 우선순위/폴백 관리:

| Capability | Providers |
|---|---|
| `web-scrape` | browser-scrape (local), firecrawl (api) |
| `web-search` | naver-search (api) |
| `keyword-analytics` | naver-ads (api) |
| `stock-trading` | korea-invest (api), kiwoom (api) |
| `notification` | kakao-talk (api) |

Provider 실행 순서: 관리자가 설정에서 직접 순서 지정. 실패 시 다음 provider로 자동 폴백.

---

## Tech Stack

| Layer | Technology |
|---|---|
| **Framework** | Next.js 16 (App Router, Turbopack) |
| **Language** | TypeScript 6 |
| **Styling** | Tailwind CSS 4 |
| **AI** | OpenAI · Anthropic · Google Gemini/Vertex (Config-driven 멀티 프로바이더) + CLI 구독 모드 |
| **Database** | SQLite (better-sqlite3) |
| **Editor** | Monaco Editor |
| **MCP** | @modelcontextprotocol/sdk 1.29 |
| **Scheduling** | node-cron |
| **Validation** | Zod |
| **Deploy** | PM2 + Nginx |

---

## Quick Start

### Prerequisites

- Node.js 20+
- Python 3.10+ (모듈 샌드박스용)

### Installation

```bash
git clone https://github.com/JRs-Master/firebat.git
cd firebat
npm install
```

### Development

```bash
npm run dev
```

`http://localhost:3000/admin`에서 어드민 페이지에 접속합니다.

### Configuration

1. **AI 모델**: 설정 > AI 탭에서 실행 모드(API/CLI) + 공급자(OpenAI/Google/Anthropic) + 모델 선택
   - **API 모드**: 각 공급자 API 키(sk-proj-…, AIza…, sk-ant-…) 또는 Vertex Service Account JSON 입력
   - **CLI 모드**: 서버 터미널에서 `claude auth login` / `codex login` / `gemini auth login` 완료 후 "상태 확인"
2. **타임존**: 설정 > 일반 탭에서 변경 (기본: Asia/Seoul)
3. **MCP 토큰**: 사이드바 > SYSTEM > Firebat MCP 서버 설정에서 토큰 생성 (외부 AI 접속용)

### Production

```bash
npm run build
pm2 start npm --name firebat -- start
```

### MCP Server

```bash
# stdio 모드 (로컬)
npm run mcp

# API 모드 — 설정에서 토큰 생성 후 /api/mcp 엔드포인트 사용
```

---

## Project Structure

```
firebat/
├── core/                    # 순수 비즈니스 로직 (I/O 없음)
│   ├── index.ts             #   FirebatCore Facade
│   ├── ports/               #   10개 Port 인터페이스
│   ├── managers/            #   11개 도메인 매니저
│   ├── types/               #   FirebatPlan, FirebatAction
│   └── capabilities.ts      #   Capability-Provider Registry
│
├── infra/                   # I/O 실행 계층 (어댑터)
│   ├── boot.ts              #   인프라 싱글톤 조립
│   ├── config.ts            #   설정 상수 중앙 관리
│   ├── storage/             #   파일 시스템 + Vault
│   ├── llm/                 #   ConfigDrivenAdapter + 8개 포맷 핸들러 (API 5 + CLI 3)
│   ├── sandbox/             #   프로세스 샌드박스
│   ├── cron/                #   node-cron 스케줄러
│   ├── database/            #   SQLite 어댑터
│   ├── log/                 #   4레벨 로깅 + JSONL
│   ├── network/             #   HTTP 통신 (SSRF 방어)
│   ├── mcp-client/          #   외부 MCP 서버 접속
│   └── auth/                #   Vault 기반 세션/API 토큰 저장
│
├── app/                     # Next.js App Router
│   ├── admin/               #   어드민 페이지 (채팅, 설정, 에디터)
│   ├── (user)/              #   사용자 페이지 (동적 렌더링)
│   └── api/                 #   API 라우트 (Primary Adapter)
│
├── mcp/                     # MCP 서버 (stdio + Streamable HTTP)
│   ├── server.ts            #   VSCode/Cursor 용 도구 정의
│   ├── stdio.ts             #   stdio 진입점 (외부 AI)
│   ├── internal-server.ts   #   User AI 전용 도구 세트
│   └── stdio-user-ai.ts     #   CLI 모드 User AI stdio 진입점
│
├── lib/                     # 유틸리티
│   ├── singleton.ts         #   Core 싱글톤 팩토리
│   ├── auth-guard.ts        #   API route 인증 가드
│   └── events.ts            #   SSE EventBus
│
├── system/                  # 시스템 영역
│   ├── services/            #   SEO, MCP 서버 등 설정 전용 서비스
│   └── modules/             #   빌트인 실행 모듈 (naver-search, naver-ads, korea-invest, kiwoom, kakao-talk, firecrawl, browser-scrape)
│
├── user/                    # 사용자 영역 (모듈, 데이터)
│   └── modules/             #   사용자 생성 모듈
│
├── docs/                    # 설계 문서 (바이블)
│   ├── FIREBAT_BIBLE.md     #   최고 등급 헌법 (정체성, 권력 분립)
│   ├── CORE_BIBLE.md        #   Core 순수성, 10개 Port, 11 Manager 아키텍처
│   ├── INFRA_BIBLE.md       #   10개 Adapter 구현 규격, 부트스트랩
│   ├── MODULE_BIBLE.md      #   모듈 시스템, Capability-Provider 패턴
│   ├── PAGESPEC_BIBLE.md    #   PageSpec 스키마 + 채팅 렌더링 규약
│   └── IO_SCHEMA_BIBLE.md   #   모듈 I/O 스키마 레퍼런스
│
└── data/                    # 런타임 데이터 (gitignore)
    ├── app.db               #   페이지 DB
    ├── vault.db             #   시크릿 저장소
    ├── cron-jobs.json       #   크론 잡 영속 저장
    └── logs/                #   앱 로그 + 학습 데이터 JSONL
```

---

## Roadmap

| Phase | Description | Status |
|---|---|---|
| **v0.1** | Core + Infra + 11 Managers + MCP + Scheduling + Vault + 멀티 LLM + CLI 모드 | Done |
| **v1.0** | 3-Tier Docker (EasyPanel) + Core/Next.js IPC 분리 | Planned |
| **v2.0** | Rust Core + gRPC + 시스템 모듈 동적 로드 | Planned |

---

## Design Documents

- **[FIREBAT_BIBLE.md](docs/FIREBAT_BIBLE.md)** — 최고 등급 헌법 (정체성, 권력 분립, JSON 교조주의)
- **[CORE_BIBLE.md](docs/CORE_BIBLE.md)** — Core 순수성, 10대 Port, 11-Manager 아키텍처, Plan-Execute 파이프라인
- **[INFRA_BIBLE.md](docs/INFRA_BIBLE.md)** — 10개 Adapter 구현 규격, 부트스트랩, 설정 상수
- **[MODULE_BIBLE.md](docs/MODULE_BIBLE.md)** — 모듈 시스템, Capability-Provider 패턴
- **[PAGESPEC_BIBLE.md](docs/PAGESPEC_BIBLE.md)** — PageSpec 스키마, 빌트인 Component, 채팅 렌더링 규약
- **[IO_SCHEMA_BIBLE.md](docs/IO_SCHEMA_BIBLE.md)** — 모듈 I/O 스키마 레퍼런스

---

<p align="center">
  <sub>Built with obsession by <a href="https://firebat.co.kr">Firebat</a></sub>
</p>
