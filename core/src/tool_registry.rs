    ...
    tools.register(ToolDefinition {
        ...
        parameters: serde_json::json!({
            ...
            "properties": {
                ...
                "pipeline": {"type": "array", "items": {"type": "object"}, "description": "executionMode=pipeline deterministic steps (EXECUTE/MCP_CALL/NETWORK_REQUEST/CONDITION/LLM_TRANSFORM/SAVE_PAGE/TOOL_CALL/FOREACH). To call a MODULE use TOOL_CALL with tool=sysmod_<name> — EXECUTE runs a file path straight in the sandbox and therefore skips input validation, account resolution and cache-key expansion (a `<param>CacheKey` argument silently never becomes rows there). FOREACH runs its `steps` once per item of `items` (a reference to a list an earlier step produced, e.g. \"$prev.orders\"); inside, $prev is the current item at the first inner step (and the previous inner result after that), while $stepN still points at the steps before the loop — that is how a loop body reaches a value fetched once, e.g. inputData:\"$prev.args\" + inputMap:{\"barsCacheKey\":\"$step0._cacheKey\"}. Use it when the count is decided at runtime — one order per row, one message per recipient. Cross-step reference: **$stepN counts from zero** ($step0 = the first step; the run log numbers them from 1, so subtract one). $prev IS the previous step's output itself (module {success,data} envelopes auto-unwrap to data) — path from there, e.g. $prev.result[0].accountSeq. Never invent wrappers like .output[]; an unresolved path fails the step. If you already know a value from a lookup this turn, bake the literal instead of a reference. Synthesis (summary/report) = one LLM_TRANSFORM step (format directives go in instruction; optional `model` pins a cheaper worker model for that step — omit for the current main model)"},
