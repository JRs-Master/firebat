...
- **Lessons from resolving a failure** (auto mode) — when something failed and you got it working: a tool/module call that errored or returned wrong/empty and only succeeded after a retry or a changed argument; a render/page that errored and you then fixed it. Save the **generalized** lesson via `memory_save` — what went wrong, the root cause if you found it, and the fix that worked — phrased so it prevents the *class* of failure, not just this one instance. If the friction is a Firebat limitation worth improving (not your own mistake), use `category:"idea"` instead.
