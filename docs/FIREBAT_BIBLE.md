...
**🚫 Self-installed Tauri 폐기 (2026-05-06)** — 외부 시니어 audit 결과 결단. v1.0 Final 단일 distribution = Self-hosted Docker. 폐기 사유: (a) 50MB 목표 비현실 (실제 150-200MB) (b) Node sidecar UX 폭탄 — 좀비 프로세스 / 포트 충돌 / 방화벽 발작 / 백신 오진 (어르신 사용자 손쓸 수 없음) (c) 진짜 가벼운 앱은 Next.js SPA 추출 + Tauri IPC 큰 frontend 재작업 → v2.0 의 별도 큰 작업.

**폐기물 (전부 청산 완료, 2026-05-06)**:
- `src-tauri/` 디렉토리 전체 — Cargo.toml + main.rs + node_sidecar.rs + tauri.conf.json + capabilities + scripts
- `docs/DEPLOY-TAURI.md` 삭제
- root `Cargo.toml` workspace members 에서 `src-tauri` 제거
- `package.json`: `@tauri-apps/api` / `@tauri-apps/cli` deps + `tauri` / `tauri:dev` / `tauri:build` / `tauri:install-cli` scripts 모두 제거
- `.gitignore`: 옛 `src-tauri/target` / `src-tauri/gen` / `src-tauri/icons/*` 라인 청산
- v2.0 재시작 시점에 fresh 재박음 (Next.js SPA + Tauri IPC 박힌 진짜 가벼운 ~15MB 앱).
...
### 제3-2항. Frontend State Management — 3 매니저 통제 (2026-05-06 박힘)
프론트엔드 UI 상태 전이는 **반드시 3개 매니저**로만 이루어진다 — 컴포넌트 내부에 복잡 상태 로직 박지 않는다.
- **`app/admin/hooks/chat-manager.ts`** — 대화 / 메시지 / 입력 / SSE chunk 상태 (reducer + invariant 박힘)
...
컴포넌트는 hook (`useChat` / `useEvents` / `useSetting`) 통해서만 상태 접근 — `useState` / `useReducer` 자체 박는 거 금지 (위 3 매니저 안에서만). 외부 상태 라이브러리 (Redux/Zustand/MobX) 도입 금지.

### 제3-3항. i18n — 자체 구현 (2026-05-10 박힘)
...
- 메시지 = `messages/ko.json` + `messages/en.json` (categories: common / login / setup / admin_chat / settings / page / sidebar). nested key (`page.reading_time`) + `{count}` placeholder.
...
v0.1 에서는 Next.js 내부에 동거하며, v1.0 Final 에서 분리 — Self-hosted Docker compose 의 별 컨테이너 + gRPC IPC. (Self-installed Tauri 는 폐기 — 위 cutover 노트 참조.)

**첫 부팅 시 SetupWizard** (vault 미박힘 시 자동, 2026-05-10 박힘) — `/login` 진입 시 `core.isAdminSetup()` 응답 false 면 SetupWizard 표시 (admin ID + password + interface lang + timezone). 비번 정책: 8자 이상 + 대/소/숫자/특수 중 3종류 (NIST 모던 + 컴플라이언스 절충). argon2id hash 박혀 vault 저장. 자동 로그인 + `/admin` redirect.

**보안 패턴** (2026-05-10 박힘):
- 비번 = argon2id hash (`core.set_admin_credentials` 박혀 vault 저장 시 자동). 검증 = `verify_admin_password` RPC (login + SettingsModal 비번 변경 둘 다 사용 — login 부작용 없는 단독 검증).
- Cookie secure = `lib/cookie-helpers.ts::isHttpsRequest` 동적 판정 (X-Forwarded-Proto 우선, fallback req.url scheme). 옛 `secure: NODE_ENV==='production'` 박힘 HTTP 접속 시 cookie 미저장 buggy fix.
- proto envelope 자동 unwrap = `lib/rust-core-proxy.ts::autoUnwrapProtoEnvelope` 박혀 BoolRequest/StringRequest/NumberRequest 의 `{value: T}` → raw T. frontend 측 unwrap 코드 박지 않음 (단일 지점 박힘).
...
### 제3항. 런타임 Sandbox 격리 (2026-05-06 박힘)
sysmod / user module 의 자식 프로세스 spawn 시 OS 레벨 격리. 어댑터별 보장 수준:

| 어댑터 | 환경 | 격리 메커니즘 |
|---|---|---|
| **`LinuxCgroupsSandbox`** | Linux native (Vultr / Docker) | cgroup v2 (cpu.max 50% / memory.max 256MB / pids.max 64) + seccomp-bpf (~60+ syscall allow + default `Errno(EPERM)`) + network namespace (`unshare(CLONE_NEWNET)`) |
...
### 제4항. Storage path traversal 방어 (2026-05-10 박힘)
`infra/src/adapters/storage.rs::resolve_safe_path` 박힘 — `is_absolute()` 거부 + `..` segment 거부 + `workspace_root.join(rel_path)` lexical normalize. **canonicalize() 박지 않음** — symlink 자동 풀어 self-hosted deploy 의 표준 패턴 (system/modules → src symlink) 박힌 게 workspace zone 밖 판정해 reject buggy. 옛 TS LocalStorageAdapter 의 path.resolve + isInsideZone 1:1 매칭. path traversal 방어 유지 + symlink 호환.

권한 부족 (Docker 외 환경 / non-root) 또는 kernel 미지원 (옛 커널) 시 격리 약화. 운영자가 어드민 UI 의 `ISandboxPort.capabilities()` 검사 결과 (kind / fs_readonly / network_deny / cpu_limit_ms / memory_limit_mb / seccomp_filter / warning) 로 현재 격리 수준 확인 가능.
...
### 제7항. 비용 통제 — Consolidation Budget Guard (2026-05-06 박힘)

6시간 cron 의 Consolidation engine 이 LLM API 오류 / 환각 무한 재시도 시 토큰 폭주 위험 → 진짜 가드 박힘:

- `ConsolidationAiHook.cost: Option<Arc<CostManager>>` 필드 (부팅 시 박음)
...
### 제8항. 데이터 보존 (Retention) — 자연 마찰 시점에 박음

현재 단일 사용자 단계엔 진짜 마찰 0 (1만 row 까지 충분). 본격 마찰 시점에 박을 정책:
...
자동매매 1년+ 운영 시점에 마찰 발견 시 박음. 현재 압력 0.

---

## 제12장: 옵저버빌리티와 장애 복원력 (Observability & Resilience, 2026-05-06 박힘)

Rust Core 전환의 핵심 운영 철학. Phase B-4 cutover 후 자연 박힘:
...
Sandbox / cgroup / seccomp / namespace 박는 단계마다 실패 시 **자식 spawn 자체는 성공**:
- cgroup write 실패 → `tracing::warn` + namespace + seccomp 만 박음
...
**v1.0 Final 비전**: Rust Core + Next.js Frontend + Self-hosted Docker. (Self-installed Tauri 폐기 — 위 cutover 노트 참조.)
...
| **모듈 실행** | 자식 프로세스 + `__updateSecrets` 영속 | tokio::process::Command — 모듈 코드 0 변경 (Node / Python sysmod 그대로). Phase B-post 에서 Linux cgroups + seccomp 격리 강화 (BIBLE Sandbox 진짜 격리) |
| **어댑터** | `infra/` 내 17개 어댑터 | `infra/src/adapters/` 16개 어댑터 (Phase B-4 cutover 후 multi-crate workspace) |
...
| **자동 update** | — | Docker image tag push (사용자 docker compose pull) |

### Phase 분해

| Phase | 작업 | 상태 |
|---|---|---|
| 0. 현재 | firebat.co.kr 옛 build 안정 운영. 큰 변경 X | 진행 중 |
...
- ✅ 회귀 검증 그물 복원 (inline tests 40+ 파일 → integration, 331 pass)
- 🟡 Audit cleanup Track A + B 완료
- 🟡 Self-hosted Docker compose 검증 (firebat.co.kr 마이그레이션)
- 🟡 본인 사용 1주+ 무사고 (Rust 위)
- → 자동매매 / 블로그 새 use case Rust 위에서 시작 가능

총 3~4개월 (1인 full-time, Tauri 폐기로 ~1개월 단축).
...
- **Vercel frontend + Self-hosted backend exe** (hybrid Self-installed 부활, 2026-05-10 박힌 v1.x 안건) — Frontend = `firebat.app` (Vercel 자동 배포, 사용자 빌드 0) + Backend = 사용자 머신 exe (Rust 단일 binary ~30-50MB). UPnP 자동 포트 매핑 + 공유기 DDNS + Let's Encrypt + Cloudflare Tunnel 박힌 fallback 으로 사용자 conscious effort 0. v1.x Vercel frontend 분산 작업과 자연 통합. 옛 Self-installed Tauri 폐기 사유 (Tauri+Node sidecar 무게 + 어르신 UX 폭탄) 와 무관 — frontend 분리라 본질적으로 다른 패턴.
- **Self-installed Tauri 재시작** — Next.js Static Export (SPA 모드) + Tauri IPC (invoke command) 박은 진짜 가벼운 데스크톱 앱 (~15MB). 옛 Node sidecar 폭탄 패턴 폐기. 위 hybrid 와 별개 — 오프라인 / air-gapped 환경 박힘 사용자 박힘.
