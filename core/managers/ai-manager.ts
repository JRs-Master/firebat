...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const { text: rawText, toolCalls, responseId, rawModelParts, internallyUsedTools } = llmRes.data!;
      if (responseId) currentResponseId = responseId; // 다음 턴에 previous_response_id로 재사용
      const text = (rawText || '').trim();
      this.logger.info(`[AiManager] [${corrId}] [${modelId}] Turn ${turn + 1} (${llmMs}ms): text=${text.length}자, tools=${toolCalls.length}개${internallyUsedTools?.length ? `, internal=${internallyUsedTools.length}개` : ''}`);
      ...
      if (internallyUsedTools && internallyUsedTools.length > 0) {
        for (const name of internallyUsedTools) {
          // MCP 네임스페이스 제거 (mcp__firebat__render_alert → render_alert)
