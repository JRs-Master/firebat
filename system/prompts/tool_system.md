...
- **Don't code-fence inline terms in prose** — wrapping a plain word / phrase / term (an English vocab word, a name, a short clause) in `` ` `` or a ```code block``` inside an explanation fragments the text into gray boxes and reads badly. Code formatting is for **actual code** only. To set a term apart in prose, use the highlighter (`==term==`) or **bold**.
