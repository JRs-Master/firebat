...
function AutoResizeIframe({ src, initialHeight }: { src: string; initialHeight?: string }) {
  ...
  const autoStyle = `<style>html,body{overflow:hidden !important;}</style>`;
  const autoScript = `<script>(function(){var id=${JSON.stringify(idRef.current)};var last=0;function measure(){var b=document.body;if(!b)return 0;return Math.max(b.scrollHeight,b.offsetHeight,Math.ceil(b.getBoundingClientRect().height));}function send(){var h=measure();if(h<=0||h===last)return;last=h;parent.postMessage({type:'iframe-resize',id:id,height:h},'*');}function attach(){if(!document.body)return;if(window.ResizeObserver)new ResizeObserver(send).observe(document.body);new MutationObserver(send).observe(document.body,{childList:true,subtree:true});send();}if(document.body)attach();else document.addEventListener('DOMContentLoaded',attach);window.addEventListener('load',send);[100,500,1500].forEach(function(t){setTimeout(send,t);});})();<\/script>`;
  const srcdoc = isFullDoc
    ? src.replace(/<\/head>/i, autoStyle + '</head>').replace(/<\/body>/i, autoScript + '</body>')
    : `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">${autoStyle}<style>*,*::before,*::after{box-sizing:border-box}html,body{margin:0;padding:8px;max-width:100vw}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:15px;line-height:1.6;color:#1e293b}canvas,svg,img,table,div{max-width:100%!important;height:auto}</style></head><body>${src}${autoScript}</body></html>`;
