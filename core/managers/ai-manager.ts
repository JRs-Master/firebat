...
- 시스템 내부 코드 설명/출력 금지.${userSection}`;
  ...
  async processWithTools(
    ...
    const planModeRule = opts?.planMode === true
      ? `\n\n## 플랜모드 ON (사용자 입력창 토글)\n사용자가 플랜모드를 켰습니다. 단순 조회·인사를 제외한 **모든 작업 요청** (모듈 실행·파일 수정·페이지 생성·스케줄 등록·복합 분석 등) 에 대해 **반드시 propose_plan 도구를 먼저 호출**해 작업 계획 카드를 제시하라. 사용자가 카드의 "실행" 버튼을 누른 뒤에야 실제 도구 호출. 단답 조회 (현재가 1건·환율 등) 는 계획 없이 바로 실행 가능.`
      : '';
    const finalSystemPrompt = contextSummary
