...
- **Proxy autoUnwrap** (2026-05-10 박힘): `lib/rust-core-proxy.ts::autoUnwrapProtoEnvelope` 박혀 BoolRequest / StringRequest / NumberRequest 의 `{value: T}` envelope 자동 raw T 변환. **단일 지점 박힘** — frontend route 측에 unwrap 코드 박지 않음. 새 RPC (Bool/String/Number 응답) 추가 시 frontend 신경 0.
...
| AuthManager | IAuthPort, IVaultPort, INotifierPort? | ✗ | 통합 인증 (세션+API 토큰), rate limit (5회 실패 → 60초 lock + Telegram 알림), lastUsedAt 추적, **argon2id hash** (set_admin_credentials 박혀 vault 저장 시 자동 hash, login + verify_admin_password 박혀 verify) — 2026-05-10 박힌 SetupWizard 첫 부팅 + SettingsModal 비번 변경 둘 다 사용 |
