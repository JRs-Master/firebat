...
> 최종 개정: 2026-07-08 (Intelligence 4 stores 용어 정착 · 렌더 채널 원칙(콘텐츠=fence/액션=도구) · AI 콘텐츠 샌드박스 · 선언형 게이트 · 카운트 현행화 — 42 컴포넌트 / 26 포트 / 27 매니저 / 24 어댑터)
...
Firebat 은 이 세 축 교집합:
- **V (Visual)** — 결과물이 시각적. 페이지·차트·테이블·카드·지도·학습 컴포넌트·라이브 피드 등 42 빌트인 컴포넌트.
- **A (Automation)** — 크론 스케줄러 + 파이프라인 + WS 상시 감시. 사용자 부재 중에도 반복 실행·외부 알림.
...
### 제5항. AI 콘텐츠 샌드박스 — 인앱 AI 도구 경계 (2026-07-01 도입)

**admin 이라도 AI 의 지시는 신뢰 불가 콘텐츠**(스크랩 웹페이지·hub 방문자·라이브러리 문서로 프롬프트 인젝션 가능) — 권한과 무관하게 AI 의 폭발 반경을 제한한다:
- **파일 도구**(read/write/delete/list/execute) = `confine_hub_path` **단일 공유 가드**(MCP + FC 양 경로) — admin 도 `user/` 화이트리스트(hub 는 `user/hub/<inst>/<sid>`). `system/`·`data/`(DB·vault)·바이너리·frontend 도달 불가.
- **`request_secret`** = 시크릿 원문을 AI 컨텍스트에 반환하지 않음(`present` bool 만 — 모듈은 sandbox env 로 받음).
- **`network_request`·referenceImage 등 AI 제어 URL** = `net_guard`(SSRF 차단 — loopback·RFC1918·169.254 메타데이터·내부 호스트명).
- UI 파일브라우저(gRPC StorageService)는 이 가드 밖 — 사람 admin 전용(admin 인증)이라 의도적으로 무관.
- **신규 규칙**: AI 가 URL/path 를 제어하는 새 경로를 추가할 때 net_guard/confine 경유가 필수.
...
### 제2항. 상세 규약
PageSpec 스키마, 42개 빌트인 Component, 채팅 블록 시스템, StockChart 등 채팅 전용 컴포넌트, AI 렌더 채널 등 모든 렌더링 규약은 **`docs/PAGESPEC_BIBLE.md`** 참조.

### 제3항. slug 한글 허용
페이지 URL slug는 한글을 허용한다 (예: `/bmi-계산기`). 모듈 폴더/파일명은 영어 kebab-case.

### 제4항. 렌더 채널 원칙 — "콘텐츠 = fence / 액션 = 도구" (2026-06-20 확정, 2026-07-02 코드 게이트)
**non-ASCII(한글 등 사람 텍스트) = 텍스트 채널(fence), 도구 인자 = ASCII 구조(slug/type/enum/id)만.** 모델이 도구호출 인자 안에서 한국어를 생성하면 degrade(옳→옵) 하는 상류 한계 + 단일 소스(content)가 메모리·회상을 공짜로 얻는 구조 우위 때문.
- 채팅 시각화 = 본문 안 ` ```firebat-render ` fence(JSON blocks) — 서버가 sanitize·검증 후 프론트가 렌더. 큰 데이터는 `dataCacheKey` 참조(서버가 캐시 전체 주입 — AI 손 복사·날조 차단).
- `render` **도구**는 code/math/diagram 전용(코드가 거부하는 `tool_mode` 게이트 — 프롬프트가 아니라 구조 강제). html 은 `render_iframe`.
- 액션 도구(search·save_page·sysmod 등)는 도구 그대로 — 이 원칙은 *표시* 채널에만 적용.
...
core/                   엔진 본체 (불가침 구역)
  ...
  ports/                Port 인터페이스 (현행 Rust: core/src/ports.rs 26 trait)
  types/                FirebatAction, Plan 스키마
  managers/             도메인 매니저 (현행 Rust: 27개 — AI, Storage, Page, Project, Module, Task, Schedule, Secret, MCP, Capability, Auth, Conversation, Media, Event, Status, Cost, Tool, Entity, Episodic, Consolidation, Template, Library, Hub, MemoryFacade, MemoryFile, SkillFile, LlmService)
...
infra/                  어댑터 레이어 (불가침 구역)
  boot.ts               어댑터 조립 팩토리 (현행 Rust: infra/src/main.rs, 24 운영 어댑터)
  config.ts             공통 설정 상수
...
MCP 서버는 Primary Adapter와 동급이며, Core 매니저만 호출한다. sysmod(도메인 분리) / render / 60+ builtin 도구 자동 등록 (ToolManager core 등록 → MCP auto-sync — FC·MCP 양 경로 단일 소스).
인증: Vault `system:internal-mcp-token` (내부 호출) + AuthManager API token (외부 사용자) 두 source 동시 검증. Claude.ai 웹 커스텀 커넥터용 `?token=` URL 인증 fallback 지원 (헤더 우선, 2026-07-01).
...
## 제11장: Intelligence 시스템 — 4 stores + Retrieval(회상) (2026-05-04 도입, 2026-06-14 용어 락, 2026-07-05 기억 재건)

쓸수록 가치 폭발하는 핵심 인프라. **용어 락(혼동 금지)**: Intelligence System = **4 stores** — 옛 "4-tier"·"메모리 시스템" 은 폐기 용어. "Memory" = data/memory 만 / "Recall" = Entity·Fact·Event 만.

### 제1항. 4 stores

| Store | 역할 | 구현 |
|---|---|---|
| **history** | 과거 대화 메시지 | `ConversationManager` — `conversations`+`conversation_messages`(admin·hub 단일 store, owner-keyed) + 임베딩 검색(`search_history`) |
| **recall** | 구조화 지식 — 대상(Entity)·시점 상태(Fact)·사건(Event). "이 대화가 지워져도 참인 것"만 | `EntityManager`+`EpisodicManager` — `entities`/`entity_facts`/`events`+m2m. semantic search + alias + **staging/confidence 승격 + supersede 값 이력** |
| **library** | 업로드 문서 RAG | `LibraryManager` — dense(E5)+sparse(BM25/FTS5)+RRF 하이브리드. **온디맨드**(`search_library`) — 얇은 `<LIBRARY_AVAILABLE>` 인덱스만 상시 |
| **memory** | 항상 지킬 규칙·교훈 ("HOW") | `MemoryFileManager` — `data/memory` .md+frontmatter. 인덱스(`<OPERATIONAL_MEMORY>`) 상시 주입, 본문 온디맨드 |

**Retrieval(회상)** = history + recall (+ 필요 시 library) 병합 → `<RETRIEVED_CONTEXT>` — 이 병합 결과를 "memory" 라 부르지 말 것. 웹 3사 auto-memory ≈ history, custom instructions ≈ memory — **recall(구조화) = 대응 없음 = 차별점.**

### 제2항. 자동 누적 — 사용자 수동 0 (기억 재건 2026-07-05)

- **자동 훅 (Episodic)**: `savePage` / `handleCronTrigger` / `generateImage` 가 자동 `saveEvent` (매니저 직접 호출 X).
- **Consolidation engine**: 6시간마다 비활성 대화 → worker 모델 후처리 → entity/fact/event JSON 추출(structured outputs 강제) → 자동 save. **재추출 워터마크**(`last_consolidated_at`) 로 같은 대화 중복 추출 0.
- **추출 판별식**: world-facts("대화가 지워져도 참인가") — 대화 활동("~요청함")·수치 시계열·시스템 내부 = HARD SKIP. `<TRACKED_ENTITIES>`(사용자의 실제 그래프) 주입 = 하드코딩 0 자가 스티어링.
- **staging → 승격**: 자율 추출 = 낮은 confidence(cron 0.5) 로 기록 → 반복 관측(seen_count) bump → 0.7 도달 시 회상 주입 자격. 명시 "기억해" = explicit 1.0. **supersede** = 같은 (entity, factType) 의 새 값이 옛 값을 퇴역(취소선 값 이력 보존).
- **중복 검출**: `dedupThreshold=0.92` cosine — idempotent.

### 제3항. 자동 retrieve

매 user 발화 → `RetrievalEngine.retrieve(query)` → history + recall 3종 병렬 검색 → `<RETRIEVED_CONTEXT>` prepend (superseded 제외·conf≥0.7 필터). library 는 자동 주입 대신 `<LIBRARY_AVAILABLE>` 인덱스 + AI 온디맨드 `search_library` (2026-06-27 전환 — 라이브러리는 ambient 기억이 아니라 의도적 레퍼런스).

### 제4항. AI 명시 호출 도구 (자율 / 수동)

- recall: `save_entity` / `save_entity_fact`(supersede·explicit 인자) / `search_entities` / `get_entity_timeline` / `search_entity_facts` / `save_event` / `search_events` / `list_recent_events`
- 통합: `search_memory`(RetrievalEngine 노출) / `memory_grep` / `search_history` / `search_library` / `consolidate_conversation`
- memory: `memory_save` / `memory_read` / `memory_list` / `memory_delete`
- ToolManager core 등록 → FC + CLI MCP 양쪽 자동.
...
### 제5항. 선언형 게이트 — 코드가 거부한다 (2026-06-30~07-07)

모듈 config 선언을 **디스패치 계층**(FC + MCP 양 경로)이 읽어 강제하는 안전망 — "프롬프트로 부탁" 이 아니라 코드가 거부:
- **`requiresApproval`** — 실주문·비가역 액션은 즉시 실행 대신 승인 카드(채팅) / cron 은 스케줄 승인 = 잡에 담긴 매매 승인.
...
| **Frontend** | Next.js + React + 42 빌트인 컴포넌트 | **동일 — 1년+ polished 보존** |
