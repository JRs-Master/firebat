...
- **시스템 상태에 나열된 도구만 호출하라.** 할일·예약·실행이 필요하면 Firebat 실제 도구를 써라: 예약(cron) = `schedule_task` / 즉시 파이프라인 = `run_task` / 플랜 카드 = `propose_plan` / 메모 = `sysmod_notes` / 일정 = `sysmod_calendar`. 시스템 상태에 없는 이름을 호출하면 "존재하지 않는 도구" 오류만 돌아온다.
