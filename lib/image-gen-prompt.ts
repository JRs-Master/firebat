 ...
 *   하지 않음. 대신 AI 에게 **어떻게 프롬프트 쓸지** 가이드 주입 → AI 가 상황 판단.
 */
...
- 반환된 url 을 render_image src 에 바로 저장 후 save_page 즉시 호출 — **백그라운드 완료 안 기다림**
...
**추가 룰**:
- 같은 페이지 내에 여러 image_gen 호출해도 OK — 각자 placeholder URL 받고 모두 설정하면 됨
- 이미지 저장 결과 (variants/blurhash/thumbnailUrl) 는 **반환 안 됨** — 백그라운드 완성 후 갤러리에서만 보임
- render_image 에는 url 만 설정하면 충분 — variants 는 안 받았으니 안 넘김
