...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const { text, toolCalls } = llmRes.data!;
      ...
      for (const tc of toolCalls) {
        this.logger.info(`[AiManager] [${corrId}] Tool: ${tc.name}`);
...
## 시스템 모듈
[시스템 모듈] 경로·입출력·capability를 참조하여 execute 도구로 호출.
같은 capability 모듈이 여러 개면 목록 순서 첫 번째 사용. 실패하면 시스템이 자동 폴백.
