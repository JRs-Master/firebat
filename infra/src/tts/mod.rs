...
impl TtsAdapter {
    ...
    /// 단일 화자의 보이스를 정한다.
    ///
    /// ⭐ **이름을 댔으면 그대로 간다.** 어댑터는 그 이름이 유효한지 판정하지 않는다 — 어느 이름이
    /// 있는지 아는 것은 provider 고, 여기 목록은 provider 의 카탈로그가 아니라 **아무도 이름을 안
    /// 댔을 때 우리가 고르는 기본**이다. 그 둘을 한 목록으로 겸하면 제품이 쓰려는 이름마다
    /// 여기 한 줄을 더하고 러스트를 배포해야 한다 — 선언으로 고칠 수 있어야 할 것이 코어에 갇힌다.
    /// 틀린 이름은 provider 가 거부하며 자기 목록을 말한다. 그게 최신이고 우리 목록은 아니다.
    ///
    /// 이름이 없을 때만 우리가 고른다 — 설정(`system:tts:voice`), 없으면 첫 보이스. 설정에는
    /// 목록 검사가 남는다: provider 를 바꾸면 저장된 값이 **다른 provider 의 이름**일 수 있고,
    /// 그건 부르는 쪽의 뜻이 아니라 남아 있던 값이다.
    ...
    async fn gemini(&self, req: &TtsRequest) -> InfraResult<TtsResult> {
        ...
        // signal_align 정렬 정확.
        if req.align {
            // ⭐ 레퍼런스가 정한 경계다. 벤더 예시는 연출 노트를 위에 두고 낭독할 글을
            //    `#### TRANSCRIPT` 아래에 둔다 — **헤딩**이라 모델이 경계로 읽는다.
            //    우리는 그 자리에 산문 한 줄을 지어 넣고 있었다("Read the following aloud,
            //    every line verbatim:"). 산문은 경계가 아니라 **또 하나의 대사 줄**이라,
            //    모델이 그걸 읽거나 그대로 따랐다:
            //      · 2026-08-30 — 내레이션이 자기 연출 지시를 소리 내어 읽었다(순서를 바꿔 덮음)
            //      · 2026-09-16 — 우리말 대사가 통째로 영어로 나왔다. 영어 문장이 대사 바로
            //        위에 있었고, 이건 TTS 엔진이 아니라 생성 모델이다
            //    한 번은 순서로, 한 번은 서문의 언어로 덮으려 했는데 둘 다 증상이었다.
            //    ⭐ 헤딩은 언어를 안 가린다 — 우리말·영어 분기가 필요 없어진다.
            // ⚠️ 경계 자체는 뺄 수 없다 — 없으면 native 가 긴 비대화 줄을 통째로 드롭한다
    ...
    for &ak in &iter_pts {
        ...
        if (p_ as isize) <= q_isize {
            ...
            if local.len() >= nbk && nbk > 0 {
                // 경계 선택 = **측정만**. 쉼이 충분하면 가장 긴 nbk 개를 뽑아 시간순으로 배치한다.
                // 기대시각(음절가중)은 쓰지 않는다 — 쉼 길이는 잰 값이고 기대시각은 추정이라, 잴 수
                // 있는 걸 두고 추정에 맞출 이유가 없다.
                //
                // 왜 바꿨나(실측 2026-07-27 TOEIC Part 1): 라벨 낭독형 오디오("A." … "The woman is …")는
                // **줄마다 쉼이 2개**(라벨 뒤·문장 뒤)라 후보가 줄 수의 2배가 된다. 옛 점수
                // `-|t-exp| + 2.0·dur` 는 앞의 긴 안내문(16초)이 기대시각을 ~1.2s 뒤로 밀어 진짜 문장
                // 경계(0.90s, 거리 1.35)가 다음 줄 라벨 뒤 쉼(0.70s, 거리 0.15)에게 0.45 vs 1.25 로 졌다.
                // → 그 줄이 1.2초 늘어나 단어가 전부 뒤로 밀리고 노래방 fill 이 발화보다 늦었다.
                // 실측 쉼: 1.27 / 0.98 / 0.95 / 0.90(진짜 경계) vs 0.70 / 0.65 / 0.65 / 0.60(라벨 뒤).
                // 긴 것 4개 = 진짜 경계 4개와 정확히 일치.
                //
                // **전역**으로 뽑는 게 핵심 — 경계마다 그리디로 고르면 앞 경계가 뒤쪽 큰 쉼을 먼저
                // 집어가 뒤가 밀린다(위 실측에서 B|C 가 C|D 쉼을 가져가는 오배정).
                let mut by_dur: Vec<usize> = (0..local.len()).collect();
                by_dur.sort_by(|&a, &b| {
                    local[b].2.partial_cmp(&local[a].2).unwrap_or(std::cmp::Ordering::Equal)
                });
                by_dur.truncate(nbk);
                by_dur.sort_unstable(); // 시간순(= local 이 이미 시간순이라 인덱스 순) 배치
                for (bi, &ci) in by_dur.iter().enumerate() {
                    chosen[p_ + bi] = Some(local[ci]);
                }
            } else {
                for bi in 0..nbk {
                    let t = lexp[bi];
...
mod align_tests {
    ...
    fn label_and_intra_paragraph_pauses_are_not_line_boundaries() {
        let parsed: Vec<(Option<String>, String)> = [
            ...
            "B. The woman is opening a laptop computer.",
            "C. Some chairs are being arranged around a table.",
            "D. Some documents are scattered across the floor.",
        ]
        .iter()
