...
매 user 발화 → `RetrievalEngine.retrieve(query)` → **5 source 병렬 검색** (search_history + searchEntities + searchEntityFacts + searchEvents + **Library RAG** — E5 dense + BM25/FTS5 sparse 하이브리드 + RRF) → 통합 `<MEMORY_CONTEXT>` 시스템 프롬프트 prepend. (Vault `system:ai-router:enabled` = AI Assistant 토글 ON 시)

AI 가 명시 도구 호출 안 해도 관련 entity/fact/event/자료 자동 컨텍스트 — "삼성전자 어떻게 됐지?" 같은 질의에 즉시 답변. AI 능동 검색은 `search_history` / `search_library` 도구.
...
| B. Rust Core 구현 | 16 어댑터 + 21 매니저 + 28 gRPC service + frontend RustCoreProxy + multi-crate workspace 분리. **hardcoding audit 7-pattern** | ✅ 완료 (2026-05-06) |
