#!/usr/bin/env node
/**
 * Firebat System Module: kiwoom-quote
 * Phase Stock-Split S2 (2026-05-14) — codegen 생성. infra/data/stock-apis-kiwoom.json 입력.
 *
 * 키움증권 시세 + 종목정보 + 업종 (현재가·호가·체결·일별주가·종목기본정보)
 * 62개 API. action = API ID 직접 호출 + params 가 request body.
 *
...
process.stdin.on('end', async () => {
  try {
    ...
    if (!appKey || !appSecret) {
      console.log(JSON.stringify({ success: false, error: 'KIWOOM_APP_KEY / KIWOOM_APP_SECRET 이 설정되지 않았습니다. 설정 > 시스템 모듈 > kiwoom-quote 에서 등록해주세요.' }));
      return;
    }
