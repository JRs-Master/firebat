...
pub enum ImageGenFormat {
    ...
    GeminiNativeImage,
    /// Vertex AI Gemini 이미지 — 향후 설정할 수 있음 (Service Account JSON 인증).
    VertexGeminiImage,
...
///   - `{ note: ... }` — 구독 기반 (Codex CLI) → None 반환 (cost 0 / 미설정)
///
/// 미정의 / 매칭 실패 시 None. 어댑터가 None 받으면 `ImageGenResult.cost_usd` 하지 않음.
pub fn compute_image_cost(config: &ImageGenModelConfig, quality: Option<&str>) -> Option<f64> {
    ...
    // quality 별 (OpenAI). quality 미설정 시 medium fallback (가장 흔한 default).
...
mod tests {
    ...
    fn compute_cost_openai_quality_based() {
        let configs = builtin_configs();
        let gpt = configs.iter().find(|c| c.id == "gpt-image-1.5").unwrap();
    ...
    fn compute_cost_cli_subscription_returns_none() {
        ...
        // 구독 기반 — pricing.note 만 설정되어 있어 None
