...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    // search_history 맥락 보강용 — 직전 user 발화 저장
    this._currentTurnPrevUserQuery = (recentHistory[0]?.content || '').trim();
    const systemContext = await this.gatherSystemContext(isDemo);
    ...
    const tools = await this.selectToolsForRequest(allTools, prompt, modelId, sessionUsedToolNames, opts?.conversationId);
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      if (sessionUsedToolNames.size > lastSessionSize) {
        turnTools = await this.selectToolsForRequest(allTools, prompt, modelId, sessionUsedToolNames, opts?.conversationId);
...
## 이전 턴 해석 원칙
히스토리에 최근 유저 질문이 포함될 수 있다. 이건 **참조용**이지 현재 답변에 포함해야 한다는 의미가 아니다.
- **현재 쿼리에 대한 답만** 작성하라. 이전 질문까지 함께 답하지 마라.
- **주제 전환**: 이전과 다른 주제면 이전은 완전히 무시하고 새로 시작. "이전에 A 였으니 A도 언급" 같은 행동 금지.
- **대명사/연속성**: 유저가 "이거/그거/다시/이어서" 같은 지시어·연속 표현을 쓸 때만 이전 턴을 해석 근거로 사용.
- 유저가 명시적으로 "A도, B도" 라고 쓰지 않았으면 두 개를 묶지 마라.
  ...
  private async selectToolsForRequest(
    ...
    if (this.isRouterEnabled()) {
      try {
        ...
        this.logger.info(`[LLMRouter] ${selected.length}/${allTools.length}개 선택 (${result.source}, cacheId=${result.cacheId}${result.previousFeedback ? `, fb=${result.previousFeedback}` : ''})`);
