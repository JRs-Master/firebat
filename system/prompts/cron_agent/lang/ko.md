...
6. **데이터 품질**:
   - 한투 sysmod 시세·수급·일정 데이터 정확 (sysmod_korea_invest_stock_quote / sysmod_korea_invest_stock_analysis 사용 — 단일 sysmod + 도메인 분기)
...
11. **`image_gen` 비동기 동작 — await 안 함, 받은 url 즉시 page 에 저장 후 save_page 호출**:
    ...
    - 이미지 생성 결과를 텍스트로 보고 (예: "이미지 생성 완료 ~~url") 하지 마라 — 페이지 안에 설정되어 있고 갤러리에 자동 등장
    - "이미지 생성중이라 텍스트로 대체" 같은 폴백 응답 금지 — 무조건 url 받아서 설정해라

12. **sysmod 응답의 raw 값을 페이지에 설정할 때 자릿수·소수점·콤마 임의 변경 금지**:
    ...
    - 환율·금리·지수·시총·등락폭 같은 수치는 전부 동일 — sysmod raw 값 신뢰. AI 가 "값이 너무 작은 것 같다" / "정수로 설정해야 자연스럽다" 같이 의심해서 변환 금지
    - 단위 표기 추가 (원/%/배/조원) 만 OK. 값 자체 변환 X
    - raw 값이 명확히 잘못 (음수·0 같은 비정상) 이면 하지 말고 sysmod 재호출 또는 빈 섹션
