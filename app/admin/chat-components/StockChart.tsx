...
export default function StockChart({ symbol, title, data, indicators = ['MA5', 'MA20'], buyPoints, sellPoints, futureSlots = 0, annotations, scale = 'linear' }: StockChartProps) {
  ...
  const waveBadge = (cx: number, py: number, label: string, color: string, dirIn: number, toRight = false) => {
    ...
    // 봐도 같은 라벨이 같은 쪽에 있다.
    // 긴 라벨("3파 목표 215,129")은 가운데 정렬이라 좌우가 선을 가로지른다 — 대각선 위에 얹히면
    // 예상선이 글자에 묻힌다(2026-07-29 사용자). 위치 규칙(꼭지 위 / 골 아래)은 그대로 두고,
    // 폭에 비례해 선에서 더 띄운다. 짧은 파동 번호는 지금처럼 바짝 붙어 귀속이 분명하다.
    const GAP = 13 + Math.min(10, Math.max(0, (w - 24) * 0.22));
    const dir = dirIn;
    let cy = py + dir * GAP;
    for (let k = 0; k < 8; k++) {
      const hit = placedLabels.some(
