...
- 옛 TS `core/ports/index.ts` → `core/src/ports.rs` (**18 trait** — Phase B-4 cutover 후 IMemoryFacadePort 추가, B-post audit cleanup A5 에서 INetworkPort 신설)
...
- 정적 43개 Core 도구 (`render`(통합), `image_gen`, `search_history`, `save_page`, `schedule_task`, `run_task`, `search_library` 등 — `core/src/tool_registry.rs`)
...
// 43 정적 도구 schema → core/src/tool_registry.rs
