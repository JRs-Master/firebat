...
> **원본 하나 (2026-08-25)** — 액션 **집합**의 원본은 정확히 한 곳이고, enum 유무가 모드를 가른다.
> · **선언 모드** (enum 없음) = 행이 원본. 검증 enum 은 런타임이 행에서 파생한다. 행 `params` 는
>   **이름 목록**(`["symbol", …]`) — 문구는 `input` 이 원본이고 스키마 응답이 실어 온다(verbatim
>   사본은 감사가 거부).
> · **merge 모드** (enum 잔존) = enum 이 원본, 행은 그 안의 id 에 대한 **주석**이다 — 파생 모듈이
>   게이트 하나를 달 때 `"actionCatalog": [{"id": "send", "approval": true}]` 한 줄이면 되고
>   나머지는 전부 파생으로 남는다. enum 밖의 행 id 는 죽은 행(감사 거부).
> · 행 `"hidden": true` = 디스패치·게이트·(파생) enum 에는 서되 검색·스키마에는 안 실린다
>   (벤더 낱말 별칭 — binance `klines`).
> · 카탈로그를 선언했는데 행을 못 읽으면 그 모듈의 **모든 호출이 닫힘으로 거부**된다 —
