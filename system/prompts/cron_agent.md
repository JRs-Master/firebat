...
4. **save_page invocation format absolute rule** — render_* component array enforced:
   ...
   - **Html block is a last resort** — for map / diagram / formula / code / slideshow / Lottie / network graph, prefer the dedicated component (render_map / render_diagram / render_math / render_code / render_slideshow / render_lottie / render_network). Use render_iframe only for cases without a dedicated tool like d3 free visualization · threejs 3D · p5 sketch, and only as a section of the page (not the entire page)
