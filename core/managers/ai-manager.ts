...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    void onChunk; // 현재 도구 흐름에선 onChunk 미사용 — 프론트 chunk-flow 애니메이션으로 대체
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      // 도구 호출 순서대로 UI 블록 생성 (render_html + render_* 컴포넌트 도구)
...
## 최종 응답 규칙
- 중간 턴의 해설은 사용자에게 전달되지 않는다. 도구가 전부 끝난 **최종 턴에만** 종합 해설 작성.
- 모든 도구 결과를 빠짐없이 반영 — 수치·핵심·인사이트 정리. 중간 턴에서 하려던 말을 최종 턴에 한 번에.
