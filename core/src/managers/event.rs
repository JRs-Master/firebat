...
// Tests 이관 — `infra/tests/event_manager_test.rs` (integration test).
// private const 사용 test 만 inline 유지 — `audit_log_capped_at_max` (uses `AUDIT_MAX`).
#[cfg(test)]
mod tests {
    use super::*;
    use crate::ports::ILogPort;
    ...
    fn audit_log_capped_at_max() {
        let logger: Arc<dyn ILogPort> = Arc::new(NoopLog);
        let mgr = EventManager::new(logger);
        for i in 0..150 {
