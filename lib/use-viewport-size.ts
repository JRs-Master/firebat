 ...
 * 흔들림 quirk 발생. 표/모달/이미지 박스 등에 vh 설정하면 스크롤 중 박스 길이가
 * 갑자기 늘어나 사용자 체감 불안정.
 *
 * **해결**: JS 로 첫 렌더 시 innerWidth/innerHeight 측정 + 픽셀 단위 저장.
 * iOS toolbar 자동 변동은 `resize` 이벤트 발화 X (layout 안 트리거) → 픽셀 박스
 * 안 흔들림. resize / orientationchange (회전 등 진짜 변동) 만 재측정.
 *
 * **반환**:
 * - 첫 렌더 시 `null` (SSR 안전 — fallback CSS 설정하여 깜빡임 회피)
 * - hydration 후 실제 px 값
 ...
 * ```
 *
 * @returns 픽셀 값 또는 null (SSR 첫 렌더 — fallback CSS 설정하여 깜빡임 회피)
 */
