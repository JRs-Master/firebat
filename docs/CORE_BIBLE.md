...
> 최종 개정: 2026-07-08 (포트 26 — ITimeseriesStorePort 추가 · 시맨틱 발견 엔진(semantic_catalog) · 파이프라인 스텝 공통 규약 · 디스패치 게이트)
...
- 옛 TS `core/index.ts` (FirebatCore facade) → 폐기. 대신 `core/src/grpc/*.rs` (gRPC service impl) 가 외부 진입점, `core/src/managers/*.rs` (27 매니저) 가 비즈니스 로직.
- 옛 TS `core/ports/index.ts` → `core/src/ports.rs` (**26 trait** — Phase B-4 cutover 후 IMemoryFacadePort 추가, B-post audit cleanup A5 에서 INetworkPort 신설, 이후 INotifier/ILibrary/IEmbedderCache/IConfig/IHub 누적, ITts(2026-06-20)·IWsApi·IWsStream(2026-07-05)·ITimeseriesStore(2026-07-06) 추가)
...
- Frontend 의 `lib/singleton.ts` `getCore()` 는 `RustCoreProxy` 를 반환. Phase B-typed (2026-05-12) 후 RustCoreProxy 는 ARGS_TABLE 기반 typed Request 매핑 후 `lib/grpc-typed-client.ts::callTypedClient` 로 `@connectrpc/connect-node` typed client 직접 호출. 옛 proto-loader / `callCore` / `invokeCore` layer 완전 폐기 (commit `96ef2a8`).
- **Proxy autoUnwrap** (2026-05-10 도입): `lib/rust-core-proxy.ts::autoUnwrapProtoEnvelope` 가 BoolRequest / StringRequest / NumberRequest 의 `{value: T}` envelope 자동 raw T 변환. **단일 지점 처리** — frontend route 측에 unwrap 코드 미작성. 새 RPC (Bool/String/Number 응답) 추가 시 frontend 신경 0.
...
Core가 Infra에게 요구하는 26개 포트 인터페이스. 모두 `core/src/ports.rs`에 정의된다 (옛 14 → 17 → 22 → 25 → 26; INotifier/ILibrary/IEmbedderCache/IConfig/IMemoryFacade/IHub 추가 후 ITts(2026-06-20) / IWsApi·IWsStream(2026-07-05) / ITimeseriesStore(2026-07-06) 추가).

| # | 포트 | 역할 |
|---|---|---|
| 1 | `IStoragePort` | 파일 읽기/쓰기/삭제/목록 조회 |
| 2 | `ILogPort` | 4레벨 로깅 (info/warn/error/debug) + category 필드 |
| 3 | `INotifierPort` | 외부 알림 채널 추상화 (Telegram / Discord / Email / Slack). fire-and-forget, adapter 가 toggle/rate-limit 책임 |
| 4 | `ISandboxPort` | 모듈 코드를 자식 프로세스로 격리 실행 |
| 5 | `ILlmPort` | AI 질의 (`ask_text` 텍스트 + `ask_with_tools` Function Calling + `ask_with_tools_streaming` + `get_model_id`). 요청별 `LlmCallOpts.model`로 모델 오버라이드 |
| 6 | `INetworkPort` | 경량 HTTP 통신 (Sandbox 없이) |
| 7 | `ICronPort` | 스케줄링 등록/해제/목록 + 타임존 관리 |
| 8 | `IDatabasePort` | SQLite CRUD (PageSpec, 비용 통계, 범용 SELECT) |
| 9 | `IVaultPort` | 시크릿 CRUD (`user:` 접두사 분리, 모듈 설정 JSON 저장) |
| 10 | `IMcpClientPort` | 외부 MCP 서버 연동 (도구 목록/호출) |
| 11 | `IConfigPort` | env / config 접근 추상화 (`FIREBAT_MCP_BASE_URL` 등 — std::env 직접 호출 격리) |
| 12 | `IAuthPort` | 세션 토큰 + API 토큰 관리 (Vault 기반) |
| 13 | `IEmbedderPort` | 텍스트 임베딩 (multilingual-e5-small). embed_query/embed_passage/cosine/버전 |
| 14 | `IEmbedderCachePort` | 임베딩 벡터 캐시 I/O (component / tool 검색 인덱스 영속화 — 옛 std::fs 직접 호출 격리) |
| 15 | `ILibraryPort` | 라이브러리 하이브리드 RAG (Reference / Source / Chunk CRUD + dense·sparse 검색) |
...
| 23 | `ITtsPort` | TTS 음성 합성 (OpenAI gpt-4o-mini-tts / Gemini TTS / 브라우저 위임) — LC 오디오 파이프라인 |
| 24 | `IWsApiPort` | WebSocket **스냅샷** 요청/응답 (`WsApiCall` = 전부 데이터: endpoint/login/preFrames/match_field/successWhen — provider 하드코드 0). REST 로 못 부르는 WS 전용 API(키움 조건검색 ka10171/2 등). 응답은 sandbox 와 공유하는 auto-cache choke-point 통과 |
| 25 | `IWsStreamPort` | WebSocket **상시 감시** (`WsStreamSpec` 선언형: subscribe/unsubscribe/realtimeMatch). watch 당 태스크 — 재연결 백오프 + vault 영속(재부팅 복원), 이벤트는 sink(EventManager topic `ws-stream:*`) → SSE·telegram fan-out |
| 26 | `ITimeseriesStorePort` | 시계열 **영구 store** (range-coverage — `data/timeseries.db`). config `timeseries` 선언 액션의 캔들 rows 흡수 + 커버 구간 계산 → 미커버 구간만 fetch(완전 커버 = 모듈 spawn 0). 소급 값 변경(배당/분할) 감지 시 시계열 무효화. 키 = `(module, action, idParams 정규화)` |

> 도구·컴포넌트 라우팅은 옛 `IToolRouterPort` 가 아니라 `core/src/managers/ai/{tool_search_index,component_search_index}.rs` 매니저 + `IEmbedderCachePort` 조합으로 구현된다 (포트 아님).
>
> 폐기: 옛 `IPromptLoaderPort` (2026-05-13 도입, 2026-05-16 폐기) — 시스템 prompt 영역이 `firebat_core::i18n` 통합 다국어 service 안으로 흡수. `system/prompts/{name}/lang/{lang}.md` 자동 scan + `prompt.{name}` namespace lookup. PromptBuilder 가 `i18n::prompt(name, None)` 직접 호출 — adapter wiring 0.
...
> ⚠️ 아래는 옛 TS 시절 예시 — 현재 코어는 Rust (core/src, infra/src). 개념 참고용. 어댑터 wiring 은 `infra/src/main.rs` 가 담당하며, `toolRouter` 필드는 폐기(도구·컴포넌트 라우팅은 core 매니저 + IEmbedderCachePort 조합). 실제 포트 목록은 위 26대 포트 표 참조.
...
### 제1-1항. 27-Manager 아키텍처 (2026-04-20 출발 → Recall 3 + Hub/Library/Template 로 23 → MemoryFacade·MemoryFile·SkillFile·LlmService 추가하며 27, 2026-07-05 재검증)
`FirebatCore`는 **얇은 라우팅 파사드**. 비즈니스 로직은 27개 도메인 매니저에 위임한다. 2026-06-26 부터는 **Core Mediator**(`core/src/core_facade.rs`)가 오케스트레이션 진입점 — cron 등 비동기 트리거가 `Core.run_cron_agent`/`run_cron_pipeline` 경유(매니저 간 직접 호출 0), Principal·owner-scope cross-cutting 의 중앙 지점.
...
| AuthManager | IAuthPort, IVaultPort, INotifierPort? | ✗ | 통합 인증 (세션+API 토큰), rate limit (5회 실패 → 60초 lock + Telegram 알림), lastUsedAt 추적, **argon2id hash** (set_admin_credentials 가 vault 저장 시 자동 hash, login + verify_admin_password 가 verify) — 2026-05-10 도입한 SetupWizard 첫 부팅 + SettingsModal 비번 변경 둘 다 사용 |
...
| **HubManager** | IHubPort (+ PageManager cascade) | ✗ | Hub (외부 위젯·방문자) 인스턴스 + send 오케스트레이션. **대화 영속은 ConversationManager 위임**(owner=`hub:<inst>:<sid>`, 2026-06-29 단일 매니저 통합). 인스턴스 삭제 시 hub-scoped page cascade |
| **MemoryFacade** | IEntityPort, IEpisodicPort | ✗ | `IMemoryFacadePort` 구현 (Entity+Episodic wrapper) — Consolidation 이 매니저 직접 의존 대신 이 포트 경유 |
| **MemoryFileManager** | IStoragePort | ✗ | Memory(data/memory) — owner-scoped .md+frontmatter CRUD + 동적 인덱스(`<OPERATIONAL_MEMORY>` 상시 주입, staging conf 필터) |
| **SkillFileManager** | IStoragePort | ✗ | Skill — .md+frontmatter (MemoryFileManager 미러), system∪owner 병합 + override/복원, `<SKILLS_AVAILABLE>` 인덱스 |
| **LlmService** | ILlmPort | ✗ | **leaf** LLM 호출(`ask_text`/`ask_with_tools`) — 오케스트레이터(Consolidation/Task/Schedule)가 AiManager 대신 이걸 호출 (orchestrator→orchestrator 위반 해소, 2026-06-26) |

> ModuleManager 는 2026-07-05 부터 IWsApiPort·IWsStreamPort 도 optional 보유(`with_ws_api`/`with_ws_stream` builder) — config `ws.actions` 선언 액션을 sandbox 대신 WS 로 라우팅, `ws.streams` 상시 감시(start/stop/list/restore_streams + vault 영속).
...
- 권한 = `hub_context::permits_tool` **단일 게이트** (FC=ai.rs effective_tools 필터 + MCP=tools/call dispatch 둘 다 동일 호출 → drift 0): ① 필수-on(핵심 sysmod — vault `system:hub:core-sysmods` 쉼표 목록, 미설정 기본 notes/calendar — + owner-scoped 내장 쓰기 save_entity/save_page/delete_* 등) / ② per-hub `allowed_sysmods` opt-in(외부 sysmod, dash 정규화) / ③ deny(Vault/시크릿/network_request/run_module/execute/admin·시스템·mcp·log 관리).
...
| 코드 어시스트 | `codeAssist` |
| **메모리 — Entity** | `saveEntity`, `updateEntity`, `deleteEntity`, `getEntity`, `findEntityByName`, `searchEntities`, `saveEntityFact`, `updateEntityFact`, `deleteEntityFact`, `getEntityFact`, `getEntityTimeline`, `searchEntityFacts`, `retrieveEntityContext`, `cleanupExpiredFacts` |
| **메모리 — Episodic** | `saveEvent`, `updateEvent`, `deleteEvent`, `getEvent`, `searchEvents`, `listRecentEvents`, `listEventsByEntity`, `linkEventEntity`, `unlinkEventEntity`, `cleanupExpiredEvents` |
| **메모리 — Consolidation** | `consolidateConversation` (대화 → LLM 추출 → 자동 save), `consolidateInactiveConversations` (cron 자동, 6시간) |
| **메모리 — 통계** | `getMemoryStats` (entities/facts/events 총수 + byType 분포, dashboard 용) |
...
"많은 정보 한번에 < 정확한 정보 조금씩" 원칙의 구현. `core/src/managers/ai/semantic_catalog.rs` = **공용 발견 엔진**(CatalogEntry + E5 임베딩 + sha1 해시 증분 캐시 + cosine top-k + lexical 부스트(정확 id/name 질의 top 고정) + multi-prefix owner 스코프).
- **카탈로그 6종**이 이 엔진 하나를 소비: components / module-actions / skills / templates / pages / media.
- **모듈 액션 계층**: 액션 수백 개 모듈(한투 278·키움 208·토스 28)은 config `actionCatalog` 선언 → `search_module_actions`(크로스-모듈 자연어 검색) → `get_action_schema`(param 설명 + 호출 봉투) → 정확 호출. 검증 에러 힌트도 이 흐름으로 안내.
- **"인덱스 = 트리거" 원칙**: 발견 표면(인덱스·검색 결과)은 트리거만 — 행동 재료(파라미터·레시피)는 get 계층에서. 검색 결과에 미니 레시피를 넣으면 모델이 get 을 건너뛰고 추측한다(2026-07-08 실측).
...
- **needs (액션 행 선언 — v3, 옛 grounding 기계 대체 2026-08-17)** — 행이 `"needs":["stock-lookup"]` 을 선언하면 판정은 "그 모듈이 이 대화(30분 슬라이딩, `conversation_scope.runs_seen`)에서 성공 실행됐나" 하나. core 는 값 모양을 모른다. cron 면제(승인 게이트와 동일). 통지는 세 계단 — search 행 `needs` · 스키마 `first`(조립된 선행 호출체) · 디스패치 거부(바닥).
...
> ⚠️ 아래는 옛 TS 시절 예시 — 현재 코어는 Rust (core/src, infra/src). 개념 참고용. 실제 타입은 `core/src/ports.rs` 의 serde struct.
...
// 정적 도구 60+ schema → core/src/tool_registry.rs
...
**스텝 호출 공통 규약 (2026-07-08, `7101f9cc`)** — EXECUTE / MCP_CALL / TOOL_CALL 은 대상만 다른 같은 계약. `core/src/managers/task.rs` 공통 함수 2개가 강제:
- `find_unresolved_ref` **fail-fast** — 인자에 미해석 `$prev`/`$stepN` 참조가 남으면 실행 전 스텝 실패(모델이 발명한 래퍼 경로가 모듈로 새지 않음). `$prev` = **이전 스텝 출력 자체** — 모듈 `{success,data}` envelope 은 자동 언랩되며 `.output` 같은 래퍼를 지어내지 말 것 (예: `$prev.result[0].accountSeq`).
- `call_outcome` — 모듈 레벨 실패(`{success:false}` envelope) = 스텝 실패로 집계(성공 시 data 언랩). 옛엔 EXECUTE 만 검사해 실주문 거절이 cron 로그 "성공" 으로 남던 비대칭.
- **MCP_CALL firebat 루프백** — `server:"firebat"`(또는 생략) 이면 외부 MCP 클라이언트 대신 내부 sysmod 디스패치로 (CLI 가 보는 이름 `mcp__firebat__*` 그대로 파이프라인에 넣어도 동작). requiresApproval 게이트(cron 허용/인터랙티브 거부) + 비활성 모듈 차단 통과.
- **unattended_module_gate** — EXECUTE 도 비활성 모듈·승인 액션을 무인 컨텍스트에서 차단 (sandbox 직행 우회 봉쇄).

---

## 제6장: Core Managers

> ⚠️ 아래는 옛 TS 시절 예시 — 현재 코어는 Rust (core/src/managers/*.rs). 개념 참고용. `process()` / `planOnly()` / `executePlan()` 같은 레거시 JSON 모드 메서드는 2026-04-22 폐기 (Function Calling `requestActionWithTools` 단일 경로). 각 매니저의 현행 역할은 제3장 27-Manager 표 참조.
...
- 사용자 query → history(search_history) + Recall(searchEntities + searchEntityFacts + searchEvents) 병렬 검색 → 통합 `<RETRIEVED_CONTEXT>` 시스템 프롬프트 자동 prepend. recall 은 superseded 제외 + confidence≥0.7 필터(staging 제외).
- **Library 는 온디맨드** (2026-06-27 전환): 자동 주입 대신 얇은 `<LIBRARY_AVAILABLE>` 인덱스(이름+설명) 상시 + AI 가 `search_library` 능동 검색 — 라이브러리는 ambient 기억이 아니라 의도적 레퍼런스.
- 상시 주입 인덱스류: `<OPERATIONAL_MEMORY>`(data/memory) / `<SKILLS_AVAILABLE>` / `<TRACKED_ENTITIES>`(사용자의 recall 그래프 — 추출 자가 스티어링) / `<LIBRARY_AVAILABLE>`.
...
```typescript
const result = await core.requestActionWithTools(prompt, history, { model: 'gemini-3.1-pro' });
```
...
Rust `core/src/managers/ai/plan_store.rs` — propose_plan 호출 시 planId 발급 + steps 저장 (30일 TTL, max 50).
...
어드민 UI 의 상태 관리를 3개 매니저로 분리. 이전엔 `useChat` 내부 7군데 흩어진 `setMessages` 호출로 상태 전이가 추적 불가 → 로봇 사라짐·빈 버블 버그 반복. Core 27-Manager 와 같이 **UI 도메인별 담당구역 명시화**.
...
| 구분 | 담당구역 | 위치 |
|---|---|---|
| Core 27-Manager | 도메인 비즈니스 로직, 포트 라우팅 | `core/managers/` |
