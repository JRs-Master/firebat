  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const text = (rawText || '').replace(
        /```(?:json|\w*)?\s*\n?\{[\s\S]*?"(?:chart|tool|functionCall|render_html|render_stock_chart|execute|run_task|schedule_task|sysmod_[\w]+|mcp_call)"[\s\S]*?\}\s*\n?```/g,
        ''
      ).replace(/\n{3,}/g, '\n\n').trim();
      ...
      // 시각화 도구 (render_html, render_stock_chart) 만 inline 블록화
      const RENDER_TOOLS = new Set(['render_html', 'render_stock_chart']);
      ...
      if (rawModelParts) {
        ...
        for (const part of rawModelParts as Array<Record<string, unknown>>) {
          if (part.text && typeof part.text === 'string' && !part.thought) {
            ...
            //  ```json\n{...render_html/render_stock_chart/execute/sysmod_.../run_task...}\n```
            const cleaned = (part.text as string).replace(
              /```(?:json|\w*)?\s*\n?\{[\s\S]*?"(?:chart|tool|functionCall|render_html|render_stock_chart|execute|run_task|schedule_task|sysmod_[\w]+|mcp_call)"[\s\S]*?\}\s*\n?```/g,
              ''
            ).replace(/\n{3,}/g, '\n\n');
            if (cleaned.trim()) turnBlocks.push({ type: 'text', text: cleaned });
      ...
      // 같은 tool이 여러 번 호출되면 각 placeholder가 순서대로 매칭되도록 pointer 사용
      ...
      for (const tb of turnBlocks) {
        ...
        } else if (name === 'render_stock_chart' && result.component) {
    ...
    try {
      switch (tc.name) {
        ...
        case 'render_stock_chart': {
          // 정형화된 주식 차트 — StockChart 컴포넌트로 바로 렌더
          return { success: true, component: 'StockChart', props: tc.args };
        }
        case 'render_html': {
          // CDN 라이브러리 자동 삽입
  ...
  private getCoreToolDefinitions(): ToolDefinition[] {
    return [
      ...
      },
      {
        ...
        description: '주식 차트 전용 컴포넌트. 정형화된 OHLCV 데이터를 넘기면 종가 라인 + 이동평균 + 거래량 + 매수/매도 포인트 박스를 자동 렌더링 (클로드급 디자인). 주식 시각화는 render_html보다 이것을 우선 사용.',
        parameters: {
          type: 'object',
          required: ['symbol', 'data'],
          properties: {
            symbol: { type: 'string', description: '종목 코드 (예: 005930)' },
            title: { type: 'string', description: '차트 제목 (예: "삼성전자 일봉")' },
            data: {
              type: 'array',
              description: 'OHLCV 시계열 (오래된 → 최신 순서)',
              items: {
                type: 'object',
            ...
            indicators: {
              type: 'array',
              description: '표시할 이동평균 (기본 [MA5, MA20])',
              items: { type: 'string', enum: ['MA5', 'MA10', 'MA20', 'MA60'] },
            },
            buyPoints: {
              type: 'array',
              description: '매수 포인트 (B1, B2 등)',
              items: {
                type: 'object',
                required: ['label', 'price'],
                properties: {
                  label: { type: 'string', description: '예: B1' },
                  price: { type: 'number', description: '매수 목표가' },
                  note: { type: 'string', description: '간단한 조건 설명' },
                },
              },
            },
            sellPoints: {
              type: 'array',
              description: '매도/손절 포인트',
              items: {
                type: 'object',
                required: ['label', 'price'],
                properties: {
                  label: { type: 'string', description: '예: S1, 손절' },
                  price: { type: 'number' },
                  note: { type: 'string' },
                },
              },
      ...
      {
        name: 'render_html',
        description: '자유 HTML 인라인 렌더링 (iframe). 정형화된 주식 차트는 render_stock_chart 사용. CDN 라이브러리 선택 시 자동으로 script 태그가 삽입되므로 html에 직접 CDN을 넣지 말고 libraries 파라미터로 선택하라.',
        parameters: {
          type: 'object',
          required: ['html'],
          properties: {
            ...
            libraries: {
              ...
              items: {
                type: 'string',
                enum: ['chartjs', 'd3', 'plotly', 'mermaid', 'leaflet', 'threejs', 'animejs', 'tailwindcss', 'katex', 'hljs', 'marked', 'cytoscape', 'mathjax', 'echarts', 'p5', 'lottie', 'datatables', 'swiper'],
              },
            },
          },
        },
