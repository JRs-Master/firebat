...
import { sanitizeBlock, sanitizeReply, isValidBlock, extractMarkdownTables } from '../utils/sanitize';
...
`
    ...
    // 마크다운 표 자동 변환 — AI 가 시스템 프롬프트 무시하고 |---| 표 그대로 출력하면
    // backend 가 추출해서 render_table block 으로 변환 + reply 에서 제거.
    // (공급자별 instruction-following 약점 우회 — 프롬프트 강제 대신 후처리로 해결)
    let workReply = finalReply;
    if (workReply) {
      const { cleanedReply, tables } = extractMarkdownTables(workReply);
      if (tables.length > 0) {
        for (const t of tables) {
          // text 블록 사이에 자연스럽게 끼워넣기 위해 마지막 text 블록 다음에 component 추가
          // (단순화: blocks 끝에 push — 순서 정확도 < 변환 자체 우선)
          blocks.push({ type: 'component', name: 'Table', props: { headers: t.headers, rows: t.rows } });
        }
        this.logger.info(`[AiManager] [${corrId}] 마크다운 표 ${tables.length}개 → render_table 자동 변환`);
        workReply = cleanedReply;
      }
    }
