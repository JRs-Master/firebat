...
모든 모듈은 `module.json`을 필수 포함하며, stdin/stdout JSON 프로토콜로 통신한다.
...
### 제1항. PageSpec 기반 렌더링
AI는 React/TSX 코드를 직접 작성하지 않는다. `SAVE_PAGE` 액션으로 **PageSpec JSON**을 DB에 저장하면,
`app/(user)/[slug]/page.tsx`가 런타임에 DB에서 조회하여 렌더링한다.
PageSpec은 `slug`, `head`(SEO/OG), `body`(Component 배열), `project`(묶음)로 구성된다.

### 제2항. 22개 빌트인 Component
AI는 아래 컴포넌트들의 배치와 데이터 바인딩을 JSON으로 선언한다.
...
| `Text` | 본문 텍스트 (마크다운 지원) |
| `Image` | 이미지 |
| `Form` | 입력 폼 (백엔드 모듈 바인딩) |
| `ResultDisplay` | 모듈 실행 결과 표시 |
| `Button` | 링크/액션 버튼 |
| `Divider` | 구분선 |
| `Table` | 데이터 테이블 |
| `Card` | 카드 컨테이너 (중첩 Component) |
| `Grid` | 그리드 레이아웃 (중첩 Component) |
| `AdSlot` | 광고 슬롯 |
| `Html` | 사용자 정의 HTML (iframe sandbox) |
| `Slider` | 슬라이더 입력 |
| `Tabs` | 탭 UI (중첩 Component) |
| `Accordion` | 아코디언 (중첩 Component) |
| `Progress` | 진행률 바 |
| `Badge` | 뱃지 |
| `Alert` | 알림/경고 메시지 |
| `List` | 순서형/비순서형 목록 |
| `Carousel` | 슬라이드 캐러셀 (중첩 Component) |
| `Countdown` | 카운트다운 타이머 |
| `Chart` | 차트 (bar/line/pie/doughnut) |
...
core/                   엔진 본체 (불가침 구역)
  ...
  managers/             9개 도메인 매니저 (AiManager, StorageManager, PageManager 등)
infra/                  어댑터 레이어 (불가침 구역)
  boot.ts               어댑터 조립 팩토리
  config.ts             공통 설정 상수
  storage/              IStoragePort + IVaultPort 구현
  llm/                  ILlmPort 구현 (Vertex AI)
...
stdio 모드(`npx tsx mcp/stdio.ts`)와 SSE 모드(`/api/mcp`)를 지원한다.
