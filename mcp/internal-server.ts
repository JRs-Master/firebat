...
**필수 사용 시점** (해당하면 반드시 먼저 호출, 조사·분석을 바로 시작하지 마라):
- 3개 이상 외부 도구 호출이 예상되는 조사·분석 (예: "A+B+C 분석해줘" / "키워드 트렌드 전략 분석" / "경쟁사 5곳 비교")
- 대규모 리포트·리서치 요청 ("~에 대해 알려줘" 중 범위 넓은 것)
- 파일 여러 개 수정·대규모 리팩토링
- 사용자가 "계획 먼저" 명시 요청
- 비용·시간 많이 드는 작업 시작 전

**건너뛰어도 되는 경우**: 단순 조회(현재가/날씨), 인사, 단일 도구 호출로 끝나는 작업, 3단계 앱 생성 플로우(suggest 사용).`,
    {
