...
- 시스템 내부 코드 설명/출력 금지.${userSection}`;
  ...
  async processWithTools(
    ...
    // 플랜모드 ON: 사용자가 명시적으로 입력창 토글 → 작업 전 propose_plan 카드 강제.
    // 시스템 프롬프트의 다른 propose_plan 예외 규칙(≤4 도구는 직행 등) 모두 무시되고 이 규칙이 최우선.
    const planModeRule = opts?.planMode === true
      ? `\n\n## ⚡ 플랜모드 ON — 최우선 규칙 (다른 propose_plan 가이드 모두 무력화)
사용자가 입력창에서 플랜모드를 켰습니다. 다음을 **반드시** 따르세요:

1. **첫 응답으로 무조건 propose_plan 도구 호출.** 다른 도구·텍스트 응답 금지.
   - 단답 조회 ("현재가 알려줘", "환율"), 인사·잡담만 예외 — 이 경우는 바로 답변.
   - 분석·전망·리포트·시각화·여러 도구 조합·파일 작성·스케줄 등록 등 **2단계 이상 작업이면 무조건 propose_plan 먼저**.

2. propose_plan 호출 시:
   - title: 작업 한 줄 요약 (예: "삼성전자 2026 실적전망 리포트")
   - steps: 각 단계별 {title, description, tool} (3~6 step 권장)
   - estimatedTime: "약 30초" / "1~2분" 등 추정
   - risks: 데이터 정확성·외부 API 호출 등 주의사항 1~2개

3. propose_plan 호출 후 **즉시 턴 종료** (다른 도구 호출 금지). 사용자가 카드의 "실행" 버튼을 누르면 별도 턴에서 실제 작업 수행.

4. 시스템 프롬프트 다른 곳의 "propose_plan 은 진짜 복합 작업(5+ 도구)에만 사용" 규칙 → 플랜모드 ON 일 땐 **완전 무시**. 위 규칙 1번이 절대 우선.`
      : '';
    const finalSystemPrompt = contextSummary
      ? systemPrompt + planModeRule + `\n\n${contextSummary}`
      : systemPrompt + planModeRule;
...
- 긴 텍스트 설명으로 제안 나열 → 반드시 suggest 도구의 UI 선택지로${bannedInternalLine}
