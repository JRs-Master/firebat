...
pub struct TtsRequest {
    ...
    pub wav: bool,
    /// BCP-47 in the SHORT form the provider's table lists ("ko", "en") — a region suffix is not
    /// in that table. Empty = the model infers it (it detects the input language on its own),
    ...
    /// voice; OpenAI has no equivalent and ignores it.
    pub language: String,
}
...
pub trait ITtsPort: Send + Sync {
    ...
    /// 단일 화자가 실제로 쓰일 보이스 — 부르는 쪽이 이름을 댔으면 그 이름 그대로(유효한지는
    /// provider 가 답한다), 안 댔으면 설정, 그것도 없으면 provider 기본. `effective_config` 과 같은 이유로 있다: **캐시키는 합성 전에
    /// 정해지는데 보이스는 합성 안에서 정해졌다.** 그래서 보이스가 키에 없었고, 설정이 바뀌어도
    /// 옛 파일이 캐시 적중으로 돌아왔다 — 한 영상 안에서 화자가 갈리는 유일한 통로였다.
    /// 합성도 이 함수로 같은 답을 얻는다(사본을 만들지 않는다).
...
pub trait IEpisodicPort: Send + Sync {
    ...
    /// owner=Some deletes only events owned by that owner (hub scope). None = no check (admin).
