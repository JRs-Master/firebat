      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            type: {
              type: 'string',
              enum: ['info', 'success', 'tip', 'accent', 'neutral'],
              description: 'info=파랑(정보), success=초록(완료/긍정 결과), tip=보라(팁/추천), accent=주황(강조/핵심 포인트), neutral=회색(일반/참고 메모)',
            },
            title: { type: ['string', 'null'], description: '제목 (불필요하면 null)' },
          },
        },
...
render_table·render_stock_chart·render_chart·render_alert·render_header·render_html 등은 오직 **function call**로만 호출한다.
**절대 금지**: \`\`\`json {"type":"render_xxx", ...} \`\`\` 형태를 답변 텍스트에 적는 것. (이건 실제 렌더링 안 됨 — 환각)
...
- 일반 정보·팁·강조·요약 → render_callout (info/success/tip/accent/neutral — 의미별 색상)
