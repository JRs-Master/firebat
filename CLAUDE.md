...
- **11개 매니저**: AiManager, StorageManager, PageManager, ProjectManager, ModuleManager, TaskManager, ScheduleManager, SecretManager, McpManager, CapabilityManager, AuthManager
...
- **자동실행 정책 (v0.1, 2026-04-13)**:
  ...
  - 자동 실행: EXECUTE, WRITE_FILE, APPEND_FILE, CANCEL_TASK, MCP_CALL, 조회류 전부
...
- **파이프라인 스케줄링 (v0.1, 2026-04-14)**:
  ...
  - 4가지 파이프라인 단계: EXECUTE, MCP_CALL, NETWORK_REQUEST, LLM_TRANSFORM
...
- **TaskManager 분리 (v0.1, 2026-04-14)**:
  ...
  - **파이프라인 단계 타입 제한**: EXECUTE, MCP_CALL, NETWORK_REQUEST, LLM_TRANSFORM 4가지만 허용. 모듈명을 타입으로 사용 금지
  - **Capability 자동 폴백**: EXECUTE 실패 시 같은 capability의 대체 provider로 자동 재시도. config.json의 capability/providerType 기반. 단독 EXECUTE(유저 모듈 테스트)은 폴백 없음 — 파이프라인 실행에서만 동작
...
- **파이프라인 등록 시 검증 (v0.1, 2026-04-14)**:
  - `validatePipeline()`: EXECUTE(path), MCP_CALL(server,tool), NETWORK_REQUEST(url), LLM_TRANSFORM(instruction) 필수 필드 검증
...
### 인증 인프라 (v0.1, 2026-04-15)
- **IAuthPort (10번째 포트)**: 세션 저장/조회/삭제/목록, VaultAuthAdapter 구현
- **AuthManager (11번째 매니저)**: 통합 토큰 관리 (세션 + API 토큰)
- **통합 세션 (AuthSession)**: `type: 'session' | 'api'`, `role: 'admin' | 'demo'`, `expiresAt?` (없으면 영구)
- **세션 토큰**: 로그인 시 `fbat_` + 32자 hex 발급, httpOnly 쿠키, 24시간 만료
- **API 토큰**: MCP 등 외부 접근용, 만료 없음, Bearer 헤더로 전달
- **MCP 토큰 이관**: SecretManager의 generateMcpToken/validateMcpToken/revokeMcpToken → AuthManager의 generateApiToken/validateApiToken/revokeApiTokens로 이관
- **proxy.ts (Edge Runtime)**: 쿠키/Bearer 존재 여부만 확인 (실제 검증은 API route에서)
  - `/api/auth` — 인증 불필요 (로그인 엔드포인트)
  - `/api/*` — 쿠키 또는 Bearer 없으면 401
  - `/admin` — 쿠키 없으면 /login 리다이렉트
- **requireAuth (lib/auth-guard.ts)**: API route 인증 가드, AuthSession 또는 NextResponse(401) 반환
  - Bearer 토큰 우선 → 쿠키 → 레거시 쿠키 호환
  - 데모 모드 확인: `auth.role === 'demo'` (기존 `isDemo(req)` 대체)
- **레거시 호환**: `firebat_admin_token` 쿠키도 마이그레이션 기간 동안 인식
- **관리자 자격증명**: SecretManager에서 AuthManager로 이관 (getAdminCredentials, setAdminCredentials)
- **Vault 저장**: `auth:session:{token}` 키에 AuthSession JSON 저장
...
- **디렉토리 구조**:
  ...
  - `system/modules/` — 시스템 모듈 (kakao-talk, browser-scrape, firecrawl)
  - `user/modules/` — 유저 모듈
...
- **현재 등록**: SEO(서비스, 7개 필드), MCP 서버(서비스), browser-scrape(모듈, 3개 필드), kakao-talk(모듈, 4개 필드), firecrawl(모듈, 2개 필드)
...
- **Provider 우선순위**: 관리자가 UI에서 드래그로 순서 지정 (기존 api-first/local-first 모드 → 사용자 정의 순서로 전환 예정)
  - resolve() 시 설정된 순서대로 시도, 실패 시 다음 provider로 폴백
  - 순서 배열을 Vault `system:capability:<id>:settings`의 `providers` 배열에 저장
...
- **어드민 UI**: 설정 모달 → 기능 탭 (capability 목록, provider 순서 변경)
- **현재 등록 provider**: `web-scrape` → browser-scrape(local) + firecrawl(api), `notification` → kakao-talk(api)
...
- `core/ports/index.ts` — 10개 Port 인터페이스 (IAuthPort 추가)
...
- `core/managers/auth-manager.ts` — AuthManager (통합 인증: 세션 토큰 + API 토큰 + 자격증명)
...
- `lib/auth-guard.ts` — API route 인증 가드 (requireAuth + isAuthError)
- `infra/auth/index.ts` — VaultAuthAdapter (Vault 기반 세션 저장, IAuthPort 구현)
- `infra/boot.ts` — 인프라 싱글톤 `getInfra()` (10개 어댑터 1회 조립, LLM lazy 초기화)
...
- **jina-reader 제거** → **Firecrawl API**로 교체 (속도·품질 우수)
- `system/modules/firecrawl/`: POST `https://api.firecrawl.dev/v1/scrape`, markdown 포맷
- **캐시 우회**: `maxAge: 0` 파라미터 (skipCache 아님 — 400 에러 발생)
- keyword 파라미터: 마크다운에서 키워드 주변 텍스트만 추출 (±500/3000자)
- config.json: capability=`web-scrape`, providerType=`api`, secrets=`["FIRECRAWL_API_KEY"]`

### 파이프라인 실행 개선 (v0.1, 2026-04-14)
- **EXECUTE 결과 이중 래핑 해제**: 모듈 출력 `{success,data:{url,title,text}}`에서 내부 `data`만 추출 → `$prev.url` 등 속성 접근 정상 동작
  - 문제: sandbox가 모듈 stdout을 InfraResult로 래핑 → `prev = res.data`가 `{success,data}`를 가리킴 → `$prev.url`이 undefined
  - 해결: `(res.data && 'success' in res.data && 'data' in res.data) ? res.data.data : res.data`
  - 폴백 provider 결과도 동일하게 언래핑 적용
- **`$prev.key` 속성 접근**: `resolveValue()`에서 `$prev.url`, `$prev.title` 등 1단계 속성 접근 지원
- **디버그 로깅**: 파이프라인 각 단계 input/output을 `log.debug()`로 기록 (500자 슬라이스)
- **EXECUTE inputData 필드 추가**: `action.inputData ?? action.mockData`로 하위 호환
- **단독 EXECUTE 결과 reply 반영**: `d?.text || d?.content || d?.data?.text` 등 자동 추출
- **LLM_TRANSFORM 환각 방지**: 시스템 프롬프트를 "데이터 추출기"로 역할 한정, 원본에 없는 내용 추가 금지

### 스피너 UX 개선 (v0.1, 2026-04-14)
- **ThinkingText 컴포넌트**: 2.5초 간격 4개 문구 자동 로테이션 (분석 중 → 전략 구상 → 도구 선택 → 계획 수립)
- step 이벤트에서 statusText 제거 — 로테이션 문구가 갑자기 바뀌는 문제 방지
- plan/result/error 이벤트만 `flushSync` 사용 (step은 자동 배칭)
