...
│   │  Task · Schedule · Secret · MCP · Capability │   │
│   │  Auth         11 Managers                    │   │
│   └──────────────┬───────────────────────────────┘   │
│                  │ Ports (Interface)                  │
│   ┌──────────────┴───────────────────────────────┐   │
...
사용자 프롬프트
    ↓
[Function Calling] Gemini 네이티브 도구 호출 (멀티턴 루프)
    ↓
[Tool Execution] 19개 Core 도구 + 동적 MCP 도구 실행
    ↓
...
- **스트리밍 응답**: `generateContentStream()` 사용, 토큰 단위 실시간 표시
- **19개 Core 도구**: 파일 CRUD, 페이지 관리, 모듈 실행, 스케줄링, 시크릿, MCP 호출, 인라인 HTML 렌더링
- **자동/수동 실행 정책**: 되돌리기 어려운 작업은 확인 요청, 나머지는 자동 실행
- **레거시 Plan-Execute 호환**: JSON 모드 + Zod 검증 파이프라인도 유지
...
├── system/                  # 시스템 모듈
│   ├── guidelines/          #   AI 가이드라인
│   └── modules/             #   빌트인 모듈 (naver-search, naver-ads, korea-invest, kiwoom, kakao-talk, firecrawl...)
│
├── user/                    # 사용자 영역 (모듈, 데이터)
│   └── modules/             #   사용자 생성 모듈
│
