...
/// ladder: `compress_history_with_search` injects related past conversations into the prompt,
/// and the model can still call `search_history` / `read_conversation` for anything older. This
/// window decides what is prepended by default, not what the turn is able to see.
///
