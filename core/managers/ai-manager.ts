...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    // 인라인 블록 — text/html을 순서대로 쌓음 (Claude 스타일 inline 렌더링용)
    const blocks: Array<{ type: 'text'; text: string } | { type: 'html'; htmlContent: string; htmlHeight?: string }> = [];
    let finalReply = '';
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      // 지금은 각 turn의 '텍스트 + render_html tool' 만 inline 블록화 (기타 도구는 블록에 미포함)
      const turnBlocks: Array<{ type: 'text'; text: string } | { type: 'html-placeholder'; toolCallIdx: number }> = [];
      if (rawModelParts) {
        let renderHtmlCount = 0; // 이 turn에서 몇 번째 render_html 호출인지
        for (const part of rawModelParts as Array<Record<string, unknown>>) {
          if (part.text && typeof part.text === 'string' && !part.thought) {
            turnBlocks.push({ type: 'text', text: part.text });
          } else if (part.functionCall && typeof part.functionCall === 'object') {
            const fc = part.functionCall as { name?: string };
            if (fc.name === 'render_html') {
              // 이 turn의 render_html 호출 index. toolCalls 배열에서 render_html만 세어서 위치 계산
              const toolCallIdx = toolCalls.findIndex((_, i) => {
                const count = toolCalls.slice(0, i + 1).filter(t => t.name === 'render_html').length;
                return toolCalls[i].name === 'render_html' && count === renderHtmlCount + 1;
              });
              if (toolCallIdx >= 0) {
                turnBlocks.push({ type: 'html-placeholder', toolCallIdx });
                renderHtmlCount++;
              }
            }
          }
      ...
      // turn 블록 → 최종 blocks로 변환 (placeholder를 실제 html로 치환)
      for (const tb of turnBlocks) {
        if (tb.type === 'text') {
          blocks.push({ type: 'text', text: tb.text });
        } else {
          const tc = toolCalls[tb.toolCallIdx];
          const result = toolResults.find(r => r.name === tc?.name && r.result !== undefined)?.result as Record<string, unknown> | undefined;
          if (result && result.success !== false && result.htmlContent) {
