...
function ComponentSwitch({ comp, standalone }: { comp: ComponentDef; standalone?: boolean }) {
  ...
  switch (type) {
    ...
    case 'Sentence':      return <SentenceComp sentence={p.sentence ?? p.original ?? p.text ?? p.english ?? p.eng} tokens={p.chunks ?? p.tokens} pattern={p.pattern} translation={p.translation} notes={p.notes ?? p.grammar ?? p.points ?? p.note ?? p.analysis} vocab={p.vocab ?? p.words} groups={p.groups ?? p.structure ?? p.phrases} />;
