...
- **an app is files, a document is blocks** — a page declares `head.kind` (`post` | `app`). A document page is a block array Firebat renders; an app page names its own source directory (`head.source`) and is served from those files, so it can be split, edited one file at a time, and run workers. The page also declares what it needs — storage, callable modules, extra script hosts, worker, fullscreen — and the framework translates that into the frame's sandbox tokens and the document's CSP. Nothing undeclared is granted, and the frame never gets `allow-same-origin`: an app talks to the server through a postMessage envelope, the browser-side twin of a module's stdin/stdout. Per-page storage lives in its own SQLite file under `data/pages/<slug>/`.

> 🇰🇷 **앱은 파일, 문서는 블록** — 페이지가 `head.kind`(`post`|`app`)를 선언합니다. 문서 페이지는 블록 배열이고, 앱 페이지는 자기 소스 디렉터리(`head.source`)를 가리켜 그 파일로 서빙됩니다 — 쪼갤 수 있고, 한 파일만 고쳐도 되고, 워커도 돕니다. 필요한 능력(저장소·호출 모듈·스크립트 호스트·워커·전체화면)도 페이지가 선언하고 프레임워크가 sandbox 토큰과 CSP 로 옮깁니다. 선언 안 한 건 안 열리고, 프레임엔 `allow-same-origin` 이 절대 안 붙습니다 — 앱은 postMessage 봉투로 서버와 말합니다(모듈의 stdin/stdout 과 같은 자리). 페이지 저장소는 `data/pages/<slug>/` 밑 자기 sqlite 파일입니다.
