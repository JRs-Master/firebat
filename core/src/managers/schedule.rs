...
pub struct ScheduleManager {
    cron: Arc<dyn ICronPort>,
    hooks: Option<ScheduleHooks>,
}

impl ScheduleManager {
    pub fn new(cron: Arc<dyn ICronPort>) -> Self {
    ...
    pub async fn schedule(
        ...
        Arc::clone(&self.cron)
            .schedule_with_spawn(job_id, target_path, opts)
            .await
    }
    ...
    pub async fn restore(&self) {
        Arc::clone(&self.cron).restore().await;
    }
    ...
    async fn run_once(
        ...
        if info.execution_mode.as_deref() == Some("agent") {
            if let Some(h) = hooks {
                ...
                // 옛 호출 (process_with_tools 단순 진입점) 은 AiRequestOpts::default 를 써서
                // cron_agent: None → LLM 이 admin chat 표준 prompt 만 받아 cron agentPrompt 를
                // 사용자 신규 요청처럼 잘못 해석 → "예약 완료" 답변만 내고 sysmod 호출 0 silent fail.
