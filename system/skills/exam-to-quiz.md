...
**시험지는 옮기는 것이지 다시 쓰는 게 아니다.** `stem`·`passage`·`choices` 텍스트는 추출된 원문을
**한 글자도 바꾸지 말고 그대로** 렌더에 넣는다. 금지: 문장 다듬기·번역·요약·쉬운 말로 바꾸기·보기
순서 변경·지문 줄이기·문제 유형 갈아치우기·비슷한 새 문항 창작. 사용자는 **그 시험지**를 풀려는
것이므로 문면이 달라지면 답도 근거도 무의미해진다(실측 2026-07-27: 정답·해설은 맞는데 문제를
임의로 개조해 시험지 구실을 못 했다).

- 원문이 어색하거나 오탈자가 있어도 **그대로 둔다**(교정 금지).
- 추출이 불완전해 문항이 깨져 보이면 고쳐 쓰지 말고 **그 문항은 빼거나** "추출 한계로 원문과 다를
  수 있음"을 덧붙인다 — 창작으로 메꾸지 말 것.
- 내가 보태는 것은 **`answerIndex`·`explanation` 둘뿐**(문면 밖의 새 정보). 해설은 자유롭게 쓴다.
