...
파이프라인 단계 type은 다음 5가지만 사용: EXECUTE, MCP_CALL, NETWORK_REQUEST, LLM_TRANSFORM, CONDITION.
...
CONDITION: 조건 검사. false면 파이프라인 정상 종료(에러 아님). 스케줄 + 조건 조합에 필수.
  field: 검사할 값 ("$prev", "$prev.price" 등), op: 비교 연산자, value: 비교값.
  op 종류: ==, !=, <, <=, >, >=, includes, not_includes, exists, not_exists.
  예: {"type":"CONDITION","field":"$prev.stck_prpr","op":"<","value":200000} → 가격이 20만 미만일 때만 다음 단계 실행.
  CONDITION은 $prev를 변경하지 않음 — 이전 단계의 결과가 다음 단계에 그대로 전달.
...
(C) 조건부 알림 (시세 조회 → 조건 충족 시에만 알림):
{"type":"SCHEDULE_TASK","description":"매일 12시 조건 체크","title":"삼성전자 가격 알림","cronTime":"0 12 * * *","pipeline":[
  {"type":"EXECUTE","path":"<시세 모듈 경로>","inputData":{"action":"price","symbol":"005930"}},
  {"type":"CONDITION","field":"$prev.stck_prpr","op":"<","value":200000},
  {"type":"EXECUTE","path":"<알림 모듈 경로>","inputMap":{"text":"삼성전자 현재가 $prev.stck_prpr원 — 매수 신호"}}
]}
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private isSimpleChat(prompt: string): boolean {
    const p = prompt.trim();
    // 짧은 인사/감정 표현만 Fast Path (엄격 화이트리스트)
    if (p.length <= 20 && /^(안녕|하이|헬로|ㅎㅇ|ㅎㅎ|ㅋㅋ|ㄱㅅ|감사|고마워|땡큐|잘자|바이|뭐해|뭐하|뭐야|누구|아니|ㅇㅋ|ㅇㅇ|ㄴㄴ|네|예|그래|좋아|ㅠ|hi|hello|hey|thanks|ok|bye)/i.test(p)) return true;
    // 도구가 필요한 키워드 (작업 동사 + 도메인 명사)
    const actionKeywords = /만들|생성|실행|삭제|저장|수정|변경|검색|조회|분석|스크랩|크롤|스케줄|예약|보내|발송|전송|파일|모듈|페이지|코드|함수|API|차트|그래프|시각화|설정|키워드|주식|주가|매수|매도|현재가|시세|잔고|체결|뉴스|블로그|쇼핑|크론|파이프|알림|카카오|네이버|업비트|법령|법률|법[^칙]|조문|판례|근로|민법|형법|상법|헌법|기준법|특별법|시행령|시행규칙|제\d+조/;
    if (actionKeywords.test(p)) return false;
    // 코드/경로 패턴이 있으면 도구 필요
    if (/\.(py|js|mjs|ts|json|html|css)\b/.test(p)) return false;
    if (/\/(user|system|api|admin)\//.test(p)) return false;
    // 숫자 + 명사 조합 (종목명+가격, 법조항 등)은 도구가 필요할 수 있음
    if (/\d+\s*(조|항|호|목|원|주|개|건)\b/.test(p)) return false;
    // 나머지 짧은 순수 대화만 (40자 이하, 질문이 아닌 경우)
    if (p.length <= 40 && !/[?？]/.test(p) && !/\d/.test(p)) return true;
    return false;
  }
  ...
  async processWithTools(
    ...
    const isDemo = opts?.isDemo ?? false;
    const llmOpts: LlmCallOpts | undefined = opts?.model || onChunk || opts?.image
      ? { ...(opts?.model ? { model: opts.model } : {}), ...(onChunk ? { onChunk } : {}), ...(opts?.image ? { image: opts.image } : {}) }
      : undefined;
    ...
    // ── 단순 대화 Fast Path: 도구 없이 askText로 빠른 응답 ──
    if (this.isSimpleChat(prompt) && !opts?.image) {
      this.logger.info(`[AiManager] [${corrId}] [${modelId}] 단순 대화 감지 → Fast Path (도구 0개)`);
      const { recentHistory, contextSummary } = this.compressHistory(history);
      const chatSystemPrompt = `Firebat User AI. 자연스럽게 한국어로 대화. 시스템 이름/정체 밝히지 마라. 안 물어본 정보를 덧붙이지 마라. 마크다운 규칙: 코드 블록은 실제 코드에만. 강조는 **볼드**.${contextSummary ? `\n\n${contextSummary}` : ''}`;
      // 도구 0개로 askWithTools 호출 → 스트리밍 + history 지원 유지
      const chatResult = await this.llm.askWithTools(prompt, chatSystemPrompt, [], recentHistory, [], llmOpts);
      const totalMs = Date.now() - startTime;
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const { text, toolCalls, rawModelParts } = llmRes.data!;
      ...
      for (const tc of toolCalls) {
        ...
        const result = await this.executeToolCall(tc, isDemo);
    ...
    return {
      ...
      reply: finalReply,
      executedActions,
      data: collectedData.length > 0 || suggestions.length > 0
        ? { ...(suggestions.length > 0 ? { suggestions } : {}), ...(collectedData.length > 0 ? { htmlItems: collectedData } : {}) }
        : undefined,
    };
  }
  ...
  private async executeToolCall(tc: ToolCall, isDemo = false): Promise<Record<string, unknown>> {
    try {
      switch (tc.name) {
        ...
        case 'set_secret': {
          const saved = this.core.setUserSecret(tc.args.name as string, tc.args.value as string);
          return saved ? { success: true } : { success: false, error: `SET_SECRET 실패: ${tc.args.name}` };
        }
        case 'run_task': {
        ...
        case 'render_html': {
          ...
          const cdnMap: Record<string, string> = {
            ...
            plotly: '<script src="https://cdn.jsdelivr.net/npm/plotly.js-dist-min"></script>',
  ...
  private buildToolSystemPrompt(systemContext: string): string {
    const userTz = this.core.getTimezone();
    return `Firebat User AI. 시스템 이름/정체 밝히지 마라. 자연스럽게 대화. 한국어로 답하라.

## 시스템 상태
${systemContext}

## 도구 사용 규칙
- 대화/인사/질문 → 도구 호출 없이 텍스트로 답하라.
...
## 시스템 모듈
시스템 모듈은 sysmod_ 접두사 도구로 직접 호출 가능 (예: sysmod_kiwoom, sysmod_naver_search).
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private get pipelineStepSchema(): JsonSchema {
    return {
      ...
      properties: {
        ...
        path: { type: 'string', description: 'EXECUTE: 모듈 경로' },
        inputData: { type: 'object', description: 'EXECUTE/NETWORK_REQUEST: 입력 데이터', additionalProperties: true },
  ...
  private getCoreToolDefinitions(): ToolDefinition[] {
    return [
      ...
      {
        name: 'schedule_task',
        description: '모듈/파이프라인 예약 실행 등록. 반복(cronTime), 1회(runAt), 지연(delaySec).',
        parameters: {
          type: 'object',
          required: ['title'],
          properties: {
            title: { type: 'string', description: '사이드바에 표시할 짧은 이름' },
            targetPath: { type: 'string', description: '단순 모듈 실행 시 경로' },
            inputData: { type: 'object', description: '모듈 입력 데이터', additionalProperties: true },
            pipeline: { type: 'array', description: '복합 작업 파이프라인', items: this.pipelineStepSchema },
            cronTime: { type: 'string', description: '반복 크론 표현식 (예: "0 9 * * *")' },
      ...
      {
        name: 'set_secret',
        description: '시크릿 값 저장 (비밀번호 아닌 설정값).',
        parameters: {
          type: 'object',
          required: ['name', 'value'],
          properties: {
      ...
      {
        name: 'render_html',
        description: '대화창에 인라인 HTML 렌더링 (iframe). 차트/그래프/지도/다이어그램 등 시각화에 사용. CDN 라이브러리 선택 시 자동으로 script 태그가 삽입되므로 html에 직접 CDN을 넣지 말고 libraries 파라미터로 선택하라.',
        parameters: {
          type: 'object',
          required: ['html'],
          properties: {
            ...
            libraries: {
              ...
              items: {
                type: 'string',
                enum: ['chartjs', 'd3', 'apexcharts', 'plotly', 'mermaid', 'leaflet', 'threejs', 'animejs', 'tailwindcss', 'katex', 'hljs', 'marked', 'cytoscape', 'mathjax', 'echarts', 'p5', 'lottie', 'datatables', 'swiper'],
              },
            },
          },
        },
