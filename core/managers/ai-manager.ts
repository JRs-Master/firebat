...
export class AiManager {
  ...
  private async compressHistoryWithSearch(
    ...
    // 최근 1 user 질문만 복원 — 대명사("이거"/"그거") 해결용.
    // assistant 본문은 여전히 제외(hallucination 방지).
    const userOnly = history.filter(h => h.role === 'user');
    const recentHistory: ChatMessage[] = userOnly.slice(-1);
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    const tools = await this.selectToolsForRequest(allTools, prompt, modelId, sessionUsedToolNames);
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      if (sessionUsedToolNames.size > lastSessionSize) {
        turnTools = await this.selectToolsForRequest(allTools, prompt, modelId, sessionUsedToolNames);
      ...
      const llmRes = await this.llm.askWithTools(prompt, finalSystemPrompt, turnTools, turnHistory, turnExchanges, turnLlmOpts);
      const llmMs = Date.now() - llmStart;
    ...
    // 라우터 캐시 score 업데이트 — 도구 사용률·응답 품질 기반
    if (this._llmRouter) {
      try {
        const routedTools = turnTools.map(t => t.name);
        const used = new Set(executedActions);
        const usedRouted = routedTools.filter(n => used.has(n)).length;
        const usageRate = routedTools.length > 0 ? usedRouted / routedTools.length : 0;
        // AI 가 실패·불가 문구 포함하면 약한 부정 신호
        const reply = finalReply.slice(0, 2000);
        const negativePhrases = ['찾지 못', '불가능', '해당 기능 없', '할 수 없', '도구가 없'];
        const negative = negativePhrases.some(p => reply.includes(p));

        const toolsCacheId = this._lastRouteCacheIds.tools;
        if (typeof toolsCacheId === 'number') {
          // 도구 사용률 30% 이상 + 네거티브 아님 → success
          if (usageRate >= 0.3 && !negative && finalReply.length > 50) {
            await this._llmRouter.recordSuccess(toolsCacheId);
          } else if (usageRate < 0.2 || negative) {
            await this._llmRouter.recordFailure(toolsCacheId);
          }
        }
        for (const cid of this._lastRouteCacheIds.components ?? []) {
          // 컴포넌트는 사용 여부로만 판정
          const usedAny = executedActions.some(a => a === 'render' || a.startsWith('render_'));
          if (usedAny) await this._llmRouter.recordSuccess(cid);
          else await this._llmRouter.recordFailure(cid);
        }
      } catch { /* score 업데이트 실패는 무시 */ }
      // 라우팅 cacheId 초기화 (다음 요청용)
    ...
    try {
      switch (tc.name) {
        ...
        case 'search_history': {
          ...
          const res = await this.core.searchConversationHistory(owner, query, {
...
## 시각화 — 반드시 function call, 절대 텍스트 금지
렌더 도구는 오직 **function call**로만 호출한다. 답변 텍스트에 JSON/코드블록/인라인으로 적으면 렌더링 안 되고 환각이다.

### 시각화 도구 2종류
1. **직접 호출 (안전망)** — 자주 쓰이고 간단한 UX:
   - \`render_alert\` (warn/error): 경고·리스크·위험
   - \`render_callout\` (info/success/tip/accent/highlight/neutral): 일반 정보·팁·강조·요약
2. **search_components + render (컴포넌트 카탈로그)** — 그 외 모든 시각화:
   - 1단계: \`search_components(query)\` 호출 → 매칭 컴포넌트들의 name·description·propsSchema 반환
   - 2단계: \`render(name, props)\` 호출 → 실제 렌더링. propsSchema 엄격 준수.
   - 컴포넌트 예: stock_chart(주식 OHLCV), chart(bar/line/pie), table(비교표), list, card, grid, badge, progress, header, countdown, image, divider, text

### 언제 무엇을
- 시계열 OHLCV → search_components("주식 차트") → render("stock_chart", ...)
- 여러 지표 나열·비교 → search_components("비교 표") → render("table", ...)
- 단일 시계열/분포 → search_components("차트 그래프") → render("chart", ...)
- 경고 → render_alert 직접
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private getCoreToolDefinitions(): ToolDefinition[] {
    return [
      ...
      {
        name: 'search_history',
        description: '과거 대화 벡터 검색. 사용자가 "전에 말한", "지난번에 물어봤던" 같이 이전 대화 참조를 암시하거나 맥락 연결이 필요할 때만 호출. 현재 대화부터 우선 매칭됨. 일반 잡담·신규 요청에는 사용 금지.',
        parameters: {
          type: 'object',
          required: ['query'],
          properties: {
  ...
  private async selectToolsForRequest(
    ...
    // Gemini 계열에만 적용 (MCP 미지원 → 토큰 절감 효과 큼)
    const isGemini = modelId.startsWith('gemini-');
    if (!isGemini) return allTools;
    if (!userQuery.trim()) return allTools;

    const { ALWAYS_INCLUDE } = await import('../../infra/llm/tool-search-index');

    // 1. Self-learning LLM 라우터 경로 (토글 on + Gemini 키 있을 때)
    if (this.isRouterEnabled()) {
      try {
        const router = this.getRouter();
        const result = await router.routeTools(userQuery, allTools, [...ALWAYS_INCLUDE, ...sessionUsedToolNames]);
        ...
        this.logger.info(`[LLMRouter] ${selected.length}/${allTools.length}개 선택 (${result.source}, cacheId=${result.cacheId})`);
