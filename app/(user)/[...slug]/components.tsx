...
import { AudioTransport, type TransportSpan } from '../../../lib/audio-transport';
...
function makeLiveDateFmt(sampleDate: string, daily: boolean): (d: Date) => string {
  ...
  return (d) => {
    const ymd = iso
      ...
      : `${d.getFullYear()}${p2(d.getMonth() + 1)}${p2(d.getDate())}`;
    return daily ? ymd : `${ymd} ${p2(d.getHours())}:${p2(d.getMinutes())}`;
  };
}

/** 실시간 봉차트 (S7) — 스트림 틱을 **현재 기간 봉**에 집계해 StockChart 렌더 코어에 태움.
 *  기간 = interval(초): 분봉/시간봉이면 시각 버킷, **일봉(interval≥86400)이면 오늘 봉 하나가
...
function LiveStockChartComp({ topic, symbol, title, data, indicators, valueField, volumeField, interval, maxCandles, annotations, futureSlots, scale, buyPoints, sellPoints, prevClose }: {
  ...
  if (fmtRef.current === null) {
    ...
    fmtRef.current = makeLiveDateFmt(seed.length > 0 ? seed[seed.length - 1].date : '', daily);
  }
  // 현재 기간 키 — 봉 date 의 숫자만 뽑아 granularity 길이로 자른 것과 비교(일=YYYYMMDD, 분=+HHMM).
  // 로컬 시각 기준(사용자 브라우저 = KST). 일봉 첫 틱 = 시드 마지막(오늘) 봉과 키 일치 → 갱신.
  ...
  useLiveTopic(topic, visible, (frame) => {
    ...
    const dotGet = (field: string): number => {
      let v: unknown = frame;
...
function KaraokeComp({ title, audioUrl, guideUrl, clickUrl, lrcUrl, lrc, offset, record = true }: {
  title?: string;
  audioUrl?: string;
  /** 같은 렌더의 스템들 — 가이드(노래 선율을 부는 악기로)·박자(클릭). 알약이 켜고 끈다. */
  guideUrl?: string;
  clickUrl?: string;
  lrcUrl?: string;
  lrc?: string;
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {([['반주와 함께', takes.mix, 'mix'], ['목소리만', takes.voice, 'voice']] as const).map(([label, url, kind]) => (
              <div key={kind} className="rounded-lg border border-slate-200 p-2">
                ...
                <div className="mt-1"><AudioTransport src={url} study={false} /></div>
              </div>
            ))}
          </div>
        )}
...
function ImageComp({
  src: srcProp, url, alt = '', caption, width, height, variants, blurhash, thumbnailUrl,
}: {
  src?: string; url?: string; alt?: string; caption?: string; width?: number; height?: number;
  variants?: ImageVariantProp[]; blurhash?: string; thumbnailUrl?: string;
}) {
  // Prop dialect tolerance — the model writes `url`/`caption` as readily as `src`/`alt`
...
/** windSpeed(m/s) → 사전 생성 마커 PNG 키 (기상청 강도 임계 — scripts/gen-typhoon-markers.mjs 와 1:1). */
...
function MapComp({
  ...
  // zoom 은 강제하고 싶을 때만 — 생략이 기본이고, 그때 보기 좋은 뷰는 렌더러 몫이다.
  // 두 개 이상은 아래 fitBounds 가 잡고, 한 곳짜리는 그 장소가 읽히는 확대로 연다
  // (2026-08-18 실측: 모델이 숫자를 고르면 척도 오독이 끼어든다 — 본사 마커가 광역 뷰에 떴다).
  const hasExplicitZoom = typeof zoom === 'number';
