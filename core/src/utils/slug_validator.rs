...
//!
//! 사용자가 page slug "demo" 박혀있고 hub slug "demo" 같이 박을 수 있던 옛 동작 (page 우선
//! 매칭으로 hub 자동 숨김) 차단. 또한 시스템 reserved keyword (api / admin / user / hub 등) 박은
//! 영역 사용 못 박게 가드.

/// 시스템 예약 slug 영역 — Next.js route / API endpoint 영역과 충돌 차단.
/// 새 예약 영역 추가 시 본 리스트에만 박음.
...
/// reserved 영역 검사 — slug 가 RESERVED_SLUGS 안 박혀있으면 Err.
pub fn check_reserved(slug: &str) -> Result<(), String> {
    ...
    if RESERVED_SLUGS.iter().any(|r| *r == normalized) {
        return Err(format!(
            "slug \"{slug}\" 는 시스템 예약 영역이라 사용 못 박습니다. (예약 영역: {})",
            RESERVED_SLUGS.join(", ")
        ));
    }
    Ok(())
