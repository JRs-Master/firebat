...
## 2. Map 렌더 — 활동 태풍 전부 한 지도에

활동 태풍이 1개든 N개든 **한 Map**에 다 그린다. 각 태풍마다 아래 markers + line + circles + cone
세트를 만들어 각 배열에 **누적**(append)한다. 여러 태풍이면 경로선·cone 색을 태풍별로 구분(예:
태풍A track `#64748b`, 태풍B track `#f59e0b`)해 겹쳐도 알아보게 한다.
