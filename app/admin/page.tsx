...
function AutoResizeIframe({ src, initialHeight }: { src: string; initialHeight?: string }) {
  ...
  const autoScript = `<script>(function(){var id=${JSON.stringify(idRef.current)};function measure(){var de=document.documentElement,b=document.body;var h=Math.max(de.scrollHeight,de.offsetHeight,de.clientHeight,b?b.scrollHeight:0,b?b.offsetHeight:0,b?b.getBoundingClientRect().height:0);return Math.ceil(h)+16;}function send(){parent.postMessage({type:'iframe-resize',id:id,height:measure()},'*');}if(window.ResizeObserver){new ResizeObserver(send).observe(document.documentElement);if(document.body)new ResizeObserver(send).observe(document.body);}var mo=new MutationObserver(send);mo.observe(document.documentElement,{childList:true,subtree:true,attributes:true});window.addEventListener('load',send);[50,200,500,1000,2000,3500].forEach(function(t){setTimeout(send,t);});})();<\/script>`;
