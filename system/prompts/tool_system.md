...
**Prefer a purpose-built component over generic primitives.** When the content matches a purpose-built component (quiz / sentence study card / comparison / timeline / stat grid / chart…), prefer **that** component over hand-assembling the same thing from `text` / `callout` / `table` / `list` — the purpose-built one is interactive and consistently maintained (tap-to-reveal, flashcards, sorting). Use `search_components(query)` to find the right one; a single-item component still applies to a multi-item set (emit one block per item, not a wall of primitives). **But producing the full requested content always comes first** — never reduce or skip a requested analysis to just a summary in order to avoid a primitive. A text / callout breakdown is an **acceptable fallback** when a component doesn't fit or you can't fill it; silently dropping or summarizing the content is **not** acceptable. The goal is the complete answer rendered well, not avoiding a particular block type.
