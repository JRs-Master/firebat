...
export class AiManager {
  ...
  private buildSystemPrompt(systemContext: string): string {
    ...
    return `Firebat User AI. 시스템 이름/정체 밝히지 마라. 자연스럽게 대화. 한국어로 답하라. 생각(thinking)도 반드시 한국어로 하라.
...
1. 4+ steps/tool calls expected
2. Strategic planning or conceptual design
3. High ambiguity or large scope
4. Deep debugging, root cause analysis

SIMPLE (flash) if: specific, bounded, 1-3 tool calls expected.`;

    const start = Date.now();
    const res = await this.llm.askText(prompt, systemPrompt, { model: 'gemini-3.1-flash-lite-preview' });
  ...
  private async resolveAutoModel(prompt: string, corrId: string): Promise<string> {
    const choice = await this.classifyComplexity(prompt, corrId);
    return choice === 'pro' ? 'gemini-3.1-pro-preview' : 'gemini-3-flash-preview';
  }
  ...
  async processWithTools(
    ...
    if (resolvedModel === 'auto') {
      if (this.isSimpleChat(prompt)) {
        resolvedModel = 'gemini-3.1-flash-lite-preview';
      } else {
        resolvedModel = await this.resolveAutoModel(prompt, corrId);
      }
    }
    ...
    if (this.isSimpleChat(prompt) && !opts?.image) {
      ...
      const { recentHistory, contextSummary } = this.compressHistory(history);
      const chatSystemPrompt = `Firebat User AI. 자연스럽게 한국어로 대화. 모든 출력(응답, 생각, 추론)을 한국어로 작성하라. Think in Korean. 시스템 이름/정체 밝히지 마라. 안 물어본 정보를 덧붙이지 마라. 마크다운 규칙: 코드 블록은 실제 코드에만. 강조는 **볼드**.${contextSummary ? `\n\n${contextSummary}` : ''}`;
  ...
  private buildToolSystemPrompt(systemContext: string): string {
    const userTz = this.core.getTimezone();
    return `Firebat User AI. 시스템 이름/정체 밝히지 마라. 자연스럽게 대화. 모든 출력(응답, 생각, 추론)을 한국어로 작성하라. Think in Korean.
