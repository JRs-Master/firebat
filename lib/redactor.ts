/**
 * 공용 토큰·시크릿 redactor — Core / infra / observability 어디서든 import.
 *
 * 일반 로직 — 특정 keyspace 분기 X. 알려진 토큰 prefix 패턴과 시크릿 키 이름 패턴
 *  으로 광범위 mask. False positive (긴 base64 문자열 etc.) 는 보수적으로 mask.
 *
 * 위치 선정:
 *   순수 함수 (의존성 0) + Core·Infra 양쪽에서 사용 → lib/ 가 적절 (BIBLE 의 neutral utility 층).
 *   기존 infra/security/token-redactor.ts 는 backward compat 으로 re-export.
 *
 * 사용:
 *   - logger.info/warn/error meta 객체에 적용 → 로그 파일·콘솔 누설 방지.
 *   - Training JSONL 에 적용 → 파인튜닝 데이터 보호.
 *   - core/utils/error-capture.ts 의 jsonl + Telegram 에서 사용 (PII 차단).
 */
