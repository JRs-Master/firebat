...
//!
//! Phase B-17 minimum: Service Account JWT signing 미저장 — 사용자가 미리 발급한 access_token 을
...
impl FormatHandler for VertexGeminiHandler {
    async fn ask_text(
        ...
        // api_key 가 access_token 으로 설정되어 있다고 가정.
        match api_key {
            Some(k) if !k.is_empty() => {
                Err(format!(
                    "Vertex Gemini access_token 직접 발급 — Phase B-17.5 jsonwebtoken JWT signing 설정된 후 활성. 임시: Vault `system:vertex:access-token` 에 미리 발급한 token 박으세요. (받은 key prefix: {})",
                    &k[..k.len().min(8)]
                ))
            }
            _ => Err(format!(
                "Vertex 인증 미설정 — Vault `{}` 박으세요 (Service Account JSON). Phase B-17.5 JWT 자동 발급 설정된 후 자동 작동.",
    ...
    async fn ask_with_tools(
        ...
        Err("Vertex Gemini Phase B-17.5 — JWT 자동 발급 설정된 후 활성".to_string())
    }
}
