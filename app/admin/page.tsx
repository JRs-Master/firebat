...
function AutoResizeIframe({ src, initialHeight }: { src: string; initialHeight?: string }) {
  ...
  // 본문 UI와 동일한 Pretendard + 베이스 스타일을 iframe 내부에 주입
  ...
  const autoScript = `<script>(function(){var id=${JSON.stringify(idRef.current)};var peak=0;function measure(){var b=document.body;if(!b)return 0;return Math.max(b.scrollHeight,b.offsetHeight,Math.ceil(b.getBoundingClientRect().height));}function send(){var h=measure();if(h<=peak)return;peak=h;parent.postMessage({type:'iframe-resize',id:id,height:h},'*');}function attach(){if(!document.body)return;if(window.ResizeObserver)new ResizeObserver(send).observe(document.body);send();}if(document.body)attach();else document.addEventListener('DOMContentLoaded',attach);window.addEventListener('load',send);[100,500,1500,3000].forEach(function(t){setTimeout(send,t);});})();<\/script><script>(function(){var last=null;function toMouse(e){if(!e.touches||e.touches.length!==1)return;var t=e.touches[0];var tg=document.elementFromPoint(t.clientX,t.clientY);if(!tg)return;tg.dispatchEvent(new MouseEvent('mousemove',{clientX:t.clientX,clientY:t.clientY,bubbles:true,cancelable:true,view:window}));last=tg;}document.addEventListener('touchstart',toMouse,{passive:true});document.addEventListener('touchmove',toMouse,{passive:true});document.addEventListener('touchend',function(){if(last){last.dispatchEvent(new MouseEvent('mouseout',{bubbles:true}));last=null;}},{passive:true});})();<\/script><script>(function(){function patch(){var done=false;if(window.Chart&&window.Chart.defaults){var d=window.Chart.defaults;d.interaction=Object.assign({},d.interaction,{mode:'nearest',intersect:false});if(d.plugins&&d.plugins.tooltip){d.plugins.tooltip.position='nearest';}d.layout=Object.assign({},d.layout,{padding:{left:4,right:12,top:10,bottom:8}});if(d.scale){d.scale.ticks=Object.assign({},d.scale.ticks,{padding:8});}if(d.scales){['x','y','y1'].forEach(function(k){if(!d.scales[k])d.scales[k]={};d.scales[k].ticks=Object.assign({},d.scales[k].ticks,{padding:6,font:{size:11}});});}d.maintainAspectRatio=false;done=true;}if(window.echarts){done=true;}return done;}if(patch())return;var tries=0;var iv=setInterval(function(){tries++;if(patch()||tries>40){clearInterval(iv);}},100);})();<\/script>`;
