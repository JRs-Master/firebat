...
중요: 각 모듈은 sysmod_{name} 전용 도구로도 호출 가능 (예: sysmod_kiwoom).`,
  ...
  server.tool(
    'list_user_modules',
    `유저가 만든 모듈(user/modules/) 목록 — 본인이 작성한 도메인 모듈 카탈로그.
반환: [{name, description, runtime, type, scope, enabled}]
사용 시점: 외부 IDE 에서 신규 모듈 작성 전 기존 패턴 학습 / 중복 방지 / 재사용 가능 모듈 발견.`,
    {},
    async () => {
      const modules = await core.getUserModules();
      return {
        content: [{ type: 'text', text: JSON.stringify(modules, null, 2) }],
      };
    },
  ...
  server.tool(
    'get_module_schema',
    `모듈 config.json 파싱 응답 — sysmod / user 모듈의 input/output schema 직접 조회.
반환: 파싱된 config (description, capability, providerType, secrets, input JSON Schema 등) 또는 {error}.
read_file('system/modules/<name>/config.json') + JSON.parse 의 1-step 단축형.
사용 시점: 외부 IDE 의 AI 가 모듈 작성 시 sysmod 입력 schema 정확 파악 (예: kiwoom 의 action·symbol·qty 타입).`,
    {
      scope: z.enum(['system', 'user']).describe('system = system/modules + system/services 검색, user = user/modules 검색'),
      name: z.string().describe('모듈 이름 (예: kiwoom, naver-search, my-auto-trade)'),
    },
    async ({ scope, name }) => {
      const config = await core.getModuleSchema(scope as 'system' | 'user', name as string);
      if (!config) {
        return { content: [{ type: 'text', text: JSON.stringify({ error: `모듈을 찾을 수 없습니다: ${scope}/${name}` }) }] };
      }
...
사용 시점: 스케줄 등록 전 현재 타임존 확인, 시각 표시 포맷팅.`,
    ...
    async () => {
      return {
        content: [{ type: 'text', text: await core.getTimezone() }],
      };
    },
  );
