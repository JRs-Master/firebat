...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      for (const tb of turnBlocks) {
        if (tb.type === 'text') {
          // 이전 턴에서 같은/거의 같은 text가 이미 있으면 스킵 (Gemini가 매 턴 commentary 재생성하는 버릇)
          const trimmed = tb.text.trim();
          if (!trimmed) continue;
          const dup = blocks.some(b => b.type === 'text' && (b.text.trim() === trimmed || b.text.includes(trimmed) || trimmed.includes(b.text.trim())));
          if (dup) continue;
  ...
  private stripToolLeakage(input: string): string {
    ...
    // 2) 주석만 있는 leftover 라인 제거
    out = out.replace(/^\s*\/\/\s*render_(?:pagespec|html|stock_chart)[^\n]*\n?/gm, '');
    // 2b) AI 셀프 보고 문구 제거 ("(차트 렌더링 완료)", "(표 생성 완료)", "[도구 호출 완료]" 등)
