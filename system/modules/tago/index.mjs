...
process.stdin.on('end', async () => {
  try {
    ...
    if (r.items.length === 0) {
      ...
      if (action === 'subway-timetable' && String(data.dailyTypeCode) === '02') {
        // Measured 2026-08-13 on eight lines: the Saturday code is documented and empty everywhere,
        // including lines whose weekday and Sunday tables are full. Answering it as "no timetable"
        // would be wrong about the trains rather than about the feed.
        notes.push('⚠️ dailyTypeCode 02 (토요일) returns zero on every line measured — 8 of 8, including lines whose 01 and 03 tables are full. The code itself is live and correct: rows echo dailyTypeCode 01 and 03 back, and no undocumented alternative works (1, 2, 3, 00, 04, 05 and SAT all return nothing). So 02 is a valid slot that was never populated. Whether 03 covers Saturday as well cannot be told from the feed — so do not present 03 as a Saturday timetable. Say the Saturday table is not published and offer 03 for what it is.');
      }
