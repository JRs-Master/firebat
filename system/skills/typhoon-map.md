...
name: typhoon-map
kind: tool-usage
description: 태풍 경로·정보를 지도에 표시하는 방법 — 태그: 태풍, 태풍경로, 태풍지도, 예상진로, 허리케인. 본문 없이 그리면 틀리는 함정이 많다 — 반드시 get_skill 로 본문을 읽고 그릴 것. 쓰지 말 것 — 일반 날씨·기온 지도(weather-map), 태풍 텍스트 브리핑만 요청할 때(지도 불요).
---
...
- ⚠️ **예측 마커 label 도 반드시 다줄로 채운다**(중심기압·최대풍속·강풍반경) — 날짜/시각만 넣으면 팝업이 빈 카드가 된다. 현재 마커와 동일 수준의 정보를 담을 것.
