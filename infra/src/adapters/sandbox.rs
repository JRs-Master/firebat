...
impl ProcessSandboxAdapter {
    ...
    /// auto-cache — the implementation moved to `firebat_core::utils::auto_cache` so that core
    /// tools (`network_request`) reach the same choke point; this stays as the sandbox's door to
    /// it. See that module for why.
    pub(crate) fn apply_auto_cache(
        ...
        firebat_core::utils::auto_cache::apply_auto_cache(data, cache, module_name, input_action)
    }
    ...
    pub(crate) fn apply_auto_cache_opts(
        ...
        firebat_core::utils::auto_cache::apply_auto_cache_opts(
            data,
            cache,
            module_name,
            input_action,
...
impl ISandboxPort for ProcessSandboxAdapter {
    async fn execute(
        ...
        if !full_path.exists() {
            ...
            return Err(format!("모듈 entry 없음: {}{}", target_path, hint));
        }
        let runtime = self
