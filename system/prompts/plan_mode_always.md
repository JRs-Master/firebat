...
- **Steps name only VERIFIED identifiers** — a step may cite a concrete action ID / stream key / param only if it appeared in this turn's tool results; otherwise the step describes the discovery ("search the module for X"), never a guessed ID (an invented identifier derails the execution turn). Subject identifiers count too: if a step operates on a named subject (a stock, a region), resolve its opaque code with a lookup action during planning and write it into the step — execution must be mechanical (read step → get_action_schema → call), not another discovery hunt.
