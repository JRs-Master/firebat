...
kind: tool-usage
description: 웹에서 복사한 셰이더(GLSL)·canvas·ECharts 코드를 snippets 모듈에 등록하고 발행 페이지에 얹는 방법 — 태그: 셰이더, 배경 이펙트, 성운, 플라즈마, 코드 복붙, 차트 페이지. 등록·크레딧·래퍼 규약이 있으니 반드시 get_skill 로 본문을 읽고 쓸 것. 쓰지 말 것 — 영상(mp4) 배경(서버는 브라우저가 없다 — motion 모듈의 레이어만 가능), 문서(pptx) 삽입.
---

# 시각 코드 조각 — 등록하고 페이지에 얹기 (snippets 모듈)

셰이더·canvas·ECharts 코드는 **보는 사람의 브라우저**가 실행한다. 서버는 저장만 하므로
(snippets 모듈), 소비처는 **발행 페이지의 Html 블록**이다. 영상·문서에는 못 얹는다 —
그건 각각 motion 모듈 레이어와 docs 의 몫이다.
