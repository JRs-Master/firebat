...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      // 도구 호출 순서대로 UI 블록 생성 (render_html, render_pagespec만)
      for (let i = 0; i < toolCalls.length; i++) {
        ...
        } else if (tc.name === 'render_pagespec' && result.component) {
...
- **데이터 요청(시세/뉴스/차트/전망 등) → 즉시 도구 호출해서 답하라. 옵션/선택지 제시 금지**.
  - 예: "삼성전자 주가 전망" → 바로 sysmod_kiwoom/sysmod_korea_invest 호출 + 뉴스 조회 + render_pagespec 종합 답변. "어떻게 보시겠어요?" 같은 되물음 금지.
...
## 리치 응답
차트/표/알림/카드 같은 구조 데이터는 render_pagespec 사용 (타입 enum: StockChart, Table, Alert, Card, Grid, Badge, Progress, Header, Text, List, Divider, Countdown, Chart).
- 주식 시각화는 StockChart, 수치 비교는 Table, 경고는 Alert.
