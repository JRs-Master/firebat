  ...
  private async executeToolCall(tc: ToolCall, isDemo = false): Promise<Record<string, unknown>> {
    try {
      switch (tc.name) {
        ...
        case 'render_html': {
          // 프론트엔드에서 iframe으로 렌더링 — HTML 내용과 높이를 전달
          return { success: true, htmlContent: tc.args.html, htmlHeight: tc.args.height || '400px' };
        }
        case 'suggest': {
...
- CDN JS 적극 활용: 데이터를 시각화하거나 인터랙티브 UI가 필요하면 CDN 라이브러리를 반드시 사용하라.
  - Chart.js: https://cdn.jsdelivr.net/npm/chart.js (바/라인/파이/도넛/레이더)
  - D3.js: https://cdn.jsdelivr.net/npm/d3@7 (복잡한 커스텀 차트, 트리맵, 네트워크)
  - ApexCharts: https://cdn.jsdelivr.net/npm/apexcharts (인터랙티브 차트, 혼합형)
  - Plotly: https://cdn.jsdelivr.net/npm/plotly.js-dist-min (3D, 히트맵, 산점도)
  - Mermaid: https://cdn.jsdelivr.net/npm/mermaid@10 (플로차트, 시퀀스 다이어그램)
  - Leaflet: https://unpkg.com/leaflet@1/dist/leaflet.js + CSS (지도)
- 완전한 HTML: <!DOCTYPE html><html><head><script src CDN></head><body>...</body></html> 전체를 전달.
- 데이터가 있으면 표(table)보다 차트를 우선 사용하라. 숫자 나열보다 시각화가 직관적이다.
- 저장용은 save_page, 즉석 시각화는 render_html 사용.
- "차트 그려줘", "그래프로 보여줘" 등 시각화 요청 시 render_html 우선 사용.
- 여러 데이터를 비교할 때 복합 차트(mixed)나 다중 축(dual-axis)을 적극 활용하라.
- 반응형 필수: width 100%, max-width, vw 단위 사용. 고정 px 폭 금지. 차트는 responsive:true 옵션. 모바일에서도 읽히는 크기.
- render_html로 먼저 보여주고, 사용자가 저장 요청하면 save_page로 페이지화하라.
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private get pipelineStepSchema(): JsonSchema {
    return {
      type: 'object',
      properties: {
        type: { type: 'string', description: 'EXECUTE | MCP_CALL | NETWORK_REQUEST | LLM_TRANSFORM', enum: ['EXECUTE', 'MCP_CALL', 'NETWORK_REQUEST', 'LLM_TRANSFORM'] },
  ...
  private getCoreToolDefinitions(): ToolDefinition[] {
    return [
      ...
      {
        name: 'render_html',
        description: '대화창에 인라인 HTML 렌더링. 차트, 그래프, 인터랙티브 시각화 등을 iframe으로 표시. CDN 라이브러리(Chart.js, D3 등) 사용 가능. 페이지 생성 없이 대화 내에서 직접 보여줄 때 사용.',
        parameters: {
          type: 'object',
          required: ['html'],
          properties: {
            html: { type: 'string', description: '렌더링할 HTML 전체 (<!DOCTYPE html>부터 </html>까지, 또는 body 내용만). CDN script 태그 포함 가능.' },
  ...
  /** 동적 도구 정의 빌드 — Core 정적 도구 + MCP 외부 도구 */
