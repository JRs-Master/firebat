...
export default function StockChart({ symbol, title, data, indicators = ['MA5', 'MA20'], buyPoints, sellPoints }: StockChartProps) {
  ...
  // 차트 영역 cap — 모바일 240px / PC 320px. SSR null 시 240 fallback.
  // 옛 = 480/320 박은 영역 → 화면 영역 절반 박힘. 사용자 보고 "엄청 커" 박은 영역.
  // 비-스크롤 모드만 적용 (가로 스크롤 모드는 SVG 자체 width=W 고정).
  // 거래량은 가격 영역의 80/280 ≈ 28.6% 비례 (옛 viewBox 비율 유지).
  const chartMaxH = useViewportMaxHeight({ mobile: 0.35, desktop: 0.45, mobileMaxPx: 240, desktopMaxPx: 320 });
  const priceChartHeight = chartMaxH ? `${chartMaxH}px` : '240px';
  const volChartHeight = chartMaxH ? `${Math.floor(chartMaxH * 80 / 280)}px` : '70px';
