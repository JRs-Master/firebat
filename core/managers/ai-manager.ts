...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      // 도구 호출 순서대로 UI 블록 생성 (render_html, render_component만)
      for (let i = 0; i < toolCalls.length; i++) {
        ...
        } else if (tc.name === 'render_component' && result.component) {
    ...
    try {
      switch (tc.name) {
        ...
        case 'render_component': {
          ...
          const propsObj = args.props ?? {};
          // Divider만 빈 props 허용 — 나머지는 실제 데이터 필수 (AI가 앞 도구 결과를 넣도록 강제)
          if (args.type !== 'Divider' && Object.keys(propsObj).length === 0) {
            return {
              success: false,
              error: `props가 비어있습니다. ${args.type} 컴포넌트는 실제 데이터가 필요합니다. 예: StockChart는 {symbol, title, data:[{date,open,high,low,close,volume},...]}, Table은 {headers, rows}, Alert은 {message, type}. 앞서 도구에서 받은 데이터를 props에 넣어 다시 호출하세요.`,
            };
          }
          return { success: true, component: args.type, props: propsObj };
        }
        case 'render_html': {
          // CDN 라이브러리 자동 삽입
...
## 리치 응답 — 컴포넌트 사용 필수
render_component 도구로 구조화 렌더링. 타입 enum: StockChart, Table, Alert, Card, Grid, Badge, Progress, Header, Text, List, Divider, Countdown, Chart.

**다음 경우 반드시 render_component 호출 (텍스트로 대체 금지):**
- 수치 3개 이상 나열 → \`Table\`
- 주가 시세·일봉/분봉 데이터 → \`StockChart\`
- 경고/주의/권장 메시지 → \`Alert\`
