...
// Tests 이관 — `infra/tests/cost_manager_test.rs` (integration test).
// `user_tz_default_seoul` 는 private fn `user_tz()` + private field `vault` 사용 → integration
// 으로 이동 못 함. 후속 — `set_user_timezone` public API 노출 후 옮기거나 폐기.
