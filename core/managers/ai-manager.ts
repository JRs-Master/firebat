...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private isSimpleChat(prompt: string): boolean {
    ...
    // 작업 키워드가 포함되면 도구 필요
    const actionKeywords = /만들|생성|실행|삭제|저장|수정|변경|검색|조회|분석|스크랩|크롤|스케줄|예약|보내|발송|전송|파일|모듈|페이지|코드|함수|API|차트|그래프|시각화|설정|키워드|주식|주가|매수|매도|뉴스|블로그|쇼핑|크론|파이프|알림|카카오|네이버|업비트|법령|법률/;
    ...
    // 나머지 짧은 대화는 단순 대화로 판별 (100자 이하)
    if (p.length <= 100 && !/\?.*\?/.test(p)) return true;
    return false;
  }
