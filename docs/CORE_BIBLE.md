...
- 옛 TS `core/ports/index.ts` → `core/src/ports.rs` (**25 trait** — Phase B-4 cutover 후 IMemoryFacadePort 추가, B-post audit cleanup A5 에서 INetworkPort 신설, 이후 INotifier/ILibrary/IEmbedderCache/IConfig/IHub 누적, ITts(2026-06-20)·IWsApi·IWsStream(2026-07-05) 추가)
...
Core가 Infra에게 요구하는 25개 포트 인터페이스. 모두 `core/src/ports.rs`에 정의된다 (옛 14 → 17 → 22 → 25; INotifier/ILibrary/IEmbedderCache/IConfig/IMemoryFacade/IHub 추가 후 ITts(2026-06-20) / IWsApi·IWsStream(2026-07-05) 추가).
...
> ⚠️ 아래는 옛 TS 시절 예시 — 현재 코어는 Rust (core/src, infra/src). 개념 참고용. 어댑터 wiring 은 `infra/src/main.rs` 가 담당하며, `toolRouter` 필드는 폐기(도구·컴포넌트 라우팅은 core 매니저 + IEmbedderCachePort 조합). 실제 포트 목록은 위 25대 포트 표 참조.
...
- 정적 49개 Core 도구 (`render`(통합), `image_gen`, `search_history`, `save_page`, `schedule_task`, `run_task`, `search_library` 등 — `core/src/tool_registry.rs`)
...
// 49 정적 도구 schema → core/src/tool_registry.rs
