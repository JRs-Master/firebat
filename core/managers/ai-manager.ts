...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  async processWithTools(
    ...
    if (this.isSimpleChat(prompt) && !opts?.image) {
      ...
      const { recentHistory, contextSummary } = this.compressHistory(history);
      const chatSystemPrompt = `Firebat User AI. 자연스럽게 한국어로 대화. 생각(thinking)도 반드시 한국어로 하라. 시스템 이름/정체 밝히지 마라. 안 물어본 정보를 덧붙이지 마라. 마크다운 규칙: 코드 블록은 실제 코드에만. 강조는 **볼드**.${contextSummary ? `\n\n${contextSummary}` : ''}`;
