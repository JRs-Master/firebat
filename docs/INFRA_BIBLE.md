...
> 최종 개정: 2026-07-08 (어댑터 24 — TimeseriesStore 추가 · openai-chat 핸들러 복원(8 포맷, SSE 스트리밍) · AI 제어 경로 net_guard/confine 절대 원칙 · sandbox 타임아웃 60초 현행화)
...
5. **AI 제어 경로 = 가드 필수** (2026-07-01): AI 가 URL 또는 path 를 제어하는 **신규 경로**는 반드시 `net_guard::is_blocked_fetch_url`(SSRF — loopback/RFC1918/169.254/내부 호스트명) 또는 `confine_hub_path`(파일 — admin=`user/` 화이트리스트, hub=세션 디렉토리) 를 경유한다. admin 권한이라도 AI 지시는 신뢰 불가 콘텐츠(프롬프트 인젝션)로 취급 — media referenceImage 외부 fetch 가 가드를 안 타던 실침해(F-A) 재발 방지 규칙.

---

## 제2장: 24개 어댑터 구현 규격 (2026-06-02 전수 검증 → 2026-07-08 재검증)

CORE_BIBLE 제2장의 26개 포트와 짝을 이루는 infra 어댑터. **운영 어댑터 24개**가 26개 포트 중 25개를 구현한다 — `SqliteMemoryAdapter` 1개가 `IEntityPort` + `IEpisodicPort` 둘을 겸하고, 나머지 `IMemoryFacadePort` 는 infra 어댑터가 아니라 core 매니저(`MemoryFacade`, `core/src/managers/memory_facade.rs`)가 구현한다 (Entity/Episodic wrapper — hexagonal 정공). 모두 `infra/src/adapters/*.rs` (LLM·image_gen 은 `infra/src/llm/` · `infra/src/image_gen/`).
...
- 2026-07-06: `SqliteTimeseriesAdapter`(ITimeseriesStorePort) — 시계열 영구 store (range-coverage) → 24

**운영 어댑터 24개**: storage / log(tracing) / sandbox / llm / network / cron / database / vault / mcp_client / auth / embedder(E5) / media / image_processor(image-rs) / image_gen / memory(Entity+Episodic) / embedder_cache / library / config / notifier(telegram) / hub / tts / ws_api / ws_stream / timeseries.
...
- **타임아웃**: 60초 (`DEFAULT_TIMEOUT_MS = 60_000`, 호출별 `opts.timeout_ms` override).
...
- **포맷 핸들러 8종** (`infra/src/llm/formats/*.rs`) — API 5(reqwest 직접 호출) + CLI 3(자식 프로세스). API 어댑터는 SDK 없이 reqwest 로 각 프로바이더 HTTP API 직접 호출:
  ...
  | `openai-chat` | reqwest (`/v1/chat/completions`, **SSE 스트리밍**) | OpenAI 호환 Chat Completions — **Upstage Solar Pro 3** 등 호환 서버 (2026-07-05 복원·정공). FC 멀티턴 = tool_exchanges 순차 페어(gemini 미러) + `repair_tool_args`(깨진 인자 관대 수리 — replay 400 차단) + `reasoning`(thinking_level→reasoning_effort) + `prompt_cache_key`(대화별 프리픽스 캐시) + `json_schema` structured outputs(worker 경로). 스트리밍 = 청크 간 idle timeout 180s(행 조기 감지, 장고 라운드 무제한) + 바이트 버퍼 UTF-8 줄 경계 |
...
- **Firebat 내부 도구 노출** (분기 축 = 모델 `features.mcpConnector` 플래그, 포맷 아님):
  - GPT/Claude (MCP connector 지원): hosted MCP 서버 URL만 전달 → 프로바이더가 Firebat MCP 서버로 직접 접속.
  - Gemini/Vertex/openai-chat(Solar): function calling — 도구 정의 inline 전달, ToolManager 가 실행.
- 타임아웃: **LLM 전용 reqwest client 분리** (2026-07-06) — read 600s·connect 10s (큰 프롬프트+추론 라운드가 공유 client 120s 에 잘리던 것 해소). 공유 120s client 는 network_request/telegram 등 유지. openai-chat 은 SSE 스트리밍 + idle 180s.
...
- config `timeseries` 선언 액션의 응답 rows(표준 OHLCV) 를 흡수 + coverage 구간 병합. 이후 같은 시계열 요청 = 커버 구간 계산 → **미커버 구간만** bounding fetch(완전 커버 = 모듈 spawn 0, "fully covered — served from the permanent store"). 훅 = sandbox choke-point(`ts_absorb_and_serve`).
- 신선도: coverage clamp = now−24h 자정 — 미완결 봉은 저장하되 커버 표시 안 함(항상 재fetch). 소급 값 변경(배당/분할 조정) 감지 시 시계열 통째 무효화·재수집. 갭 fetch 빈 응답(주말/휴장) = store 서빙 fallback.
- 키 = `(module, action, idParams 정규화)` — 대화·owner 무관 서버 전역 공유(cron·hub 포함). bypass = limit 호출·범위 비명시·미선언.
...
| 시계열 — series / coverage / rows (`data/timeseries.db`) | `infra/src/adapters/timeseries.rs::initialize()` | `CREATE TABLE IF NOT EXISTS` |
...
`firebat-core` binary 가 시작 시 24개 운영 어댑터를 1회 wiring 하여 매니저·gRPC 서비스에 주입한다.
...
| (LLM HTTP client) | read 600s / connect 10s | LLM 전용 reqwest client (2026-07-06 분리 — 공유 120s client 는 network/telegram 용) |
| `DEFAULT_TIMEOUT_MS` (sandbox) | `60000` | 샌드박스 타임아웃 (어댑터 const, opts.timeout_ms override) |
...
옛 v2.0 의 "infra → system/modules 동적 로드" 노선 폐기. 단일 v1.0 Final milestone 으로 통합 — Rust Core 의 단일 binary 안에 24개 운영 어댑터 정적 컴파일.
