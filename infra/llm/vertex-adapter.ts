import { ILlmPort, LlmCallOpts } from '../../core/ports';
...
export class VertexAiAdapter implements ILlmPort {
  ...
  private getClient(): GoogleGenAI | null {
    ...
    if (apiKey !== this.cachedApiKey) {
      ...
      this.ai = project
        ? new GoogleGenAI({ vertexai: true, project, location, apiKey } as any)
        : new GoogleGenAI({ apiKey, vertexai: true } as any);
  ...
  async ask(prompt: string, systemPrompt?: string, history: any[] = [], opts?: LlmCallOpts): Promise<InfraResult<any>> {
