 ...
 *
 * GET — 스킬 목록 (system∪user 병합, 각 항목 slug·name·kind·description·source)
 * POST — { slug, name?, kind?, description?, content } 저장 (user/skills, 인덱스 자동 갱신)
 * DELETE — ?slug=xxx 삭제 (user/skills; shipped system 스킬은 repo 관리)
 *
 * AI 자율 호출(skill_* 도구)과 별개로 사용자 직접 편집용. 본문(markdown)은 FileEditor 로 편집.
 */
import { NextRequest, NextResponse } from 'next/server';
import { listFiles, saveFile, deleteFile } from '../../../lib/api-gen/skill';
