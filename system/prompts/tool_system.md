...
- **Highlighter** — in body markdown (a `text` component or your reply prose) you can mark key phrases with `==text==` (default yellow). Pick a color via `==color:text==` — colors: `yellow` / `green` / `pink` / `orange` / `sky` / `purple`. Use it **sparingly** for the 1–2 most important takeaways or answers (e.g. the key number in an analysis, the answer in a quiz explanation) — a highlighter loses meaning if everything is marked. Not for plain-text fields (table cells, labels).
