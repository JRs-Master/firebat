...
## 도구 사용
- 인사/잡담 → 도구 없이 텍스트로 응답.
- **데이터 요청(시세/뉴스/차트/전망 등) → 즉시 도구 호출해서 답하라. 옵션/선택지 제시 금지**.
  - 예: "삼성전자 주가 전망" → 바로 sysmod_kiwoom/sysmod_korea_invest 호출 + 뉴스 조회 + render_component 종합 답변. "어떻게 보시겠어요?" 같은 되물음 금지.
  - "주가 전망", "분석", "어때?" 같은 포괄 요청도 **임의로 쪼개지 말고 바로 필요한 데이터 전부 조회 → 한 번에 종합 해설**.
- 외부 데이터 필요 → 지식이 아닌 도구 결과로 답하라. 이전 답변이 있어도 새로 호출.
- 작업 요청 → 적절한 도구 호출.
...
- 시스템 내부 코드 설명/출력 금지.`;
  ...
  private getCoreToolDefinitions(): ToolDefinition[] {
    return [
      ...
      {
        name: 'render_html',
        description: '자유 HTML 인라인 렌더링 (iframe). 정형화된 UI는 render_component 우선 사용. render_html은 지도/다이어그램/애니메이션/수학식 등 CDN 라이브러리 필요할 때만. CDN은 libraries 파라미터로 선택.',
        parameters: {
          type: 'object',
          required: ['html'],
          properties: {
