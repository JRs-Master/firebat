...
- 시스템 내부 코드 설명/출력 금지.${userSection}`;
  ...
  async processWithTools(
    ...
    // 시스템 프롬프트의 다른 propose_plan 예외 규칙 모두 무력화. 시스템 프롬프트 맨 앞에 prepend 해서 우선순위 보장.
    const planModePrefix = opts?.planMode === true
      ? `# ⚡ 플랜모드 ON (사용자 입력창 토글) — 다른 모든 규칙보다 우선

사용자가 플랜모드를 켰습니다. **첫 응답은 무조건 propose_plan 도구 호출**.
- 인사·단순 조회(현재가 1건·환율 등) 만 예외, 그 외 **모든 작업 (분석·전망·리포트·시각화·파일·스케줄 등) 2단계 이상이면 propose_plan 먼저**
- propose_plan 호출 후 **즉시 턴 종료** — 다른 도구·텍스트 응답 금지
- 사용자가 카드의 "✓ 실행" 누르면 별도 턴에서 실제 작업 진행
- 시스템 프롬프트 다른 곳의 propose_plan 가이드 (≤4 도구 예외 등) 모두 무력화

propose_plan 인자: { title (작업 요약), steps (3~6 단계 {title, description, tool?}), estimatedTime, risks }
...
`
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const promptForLlm = (opts?.planMode === true && toolExchanges.length === 0)
        ? `[플랜모드 ON — 단순 조회·인사 외엔 propose_plan 도구 먼저 호출하고 즉시 턴 종료]\n\n${prompt}`
        : prompt;
