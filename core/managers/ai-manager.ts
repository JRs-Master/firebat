...
`
    ...
    // - 플랜 토글 OFF 시: propose_plan 도구 자체 제거 → AI 가 자체 판단으로 plan 카드 띄우는 것 차단
    const allTools = allToolsRaw.filter(t => {
      if (this.isRouterEnabled() && t.name === 'search_history') return false;
      if (opts?.planMode !== true && t.name === 'propose_plan') return false;
      return true;
    });
